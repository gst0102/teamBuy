...
`POST /api/wecom/kf/teamBuy/callback` keeps mock fixture import while `WECOM_USE_MOCK=true`; when `WECOM_USE_MOCK=false`, a callback queues a background real-sync task and returns quickly. The background task uses the same cursor, lock, media transfer, and compensation pipeline as manual real-sync.
