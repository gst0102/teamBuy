...
`POST /api/wecom/callback` keeps mock fixture import while `WECOM_USE_MOCK=true`; when `WECOM_USE_MOCK=false`, a callback triggers the same real-sync pipeline so incoming customer-service events automatically enter cursor, lock, media transfer, and compensation handling.
