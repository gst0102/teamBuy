...
from app.schemas.imports import ClaimImportRequest, ClaimImportTokenRequest
...
@router.post("/claim-by-token", response_model=ApiResponse[dict])
def claim_import_by_token(payload: ClaimImportTokenRequest, service: AppService = Depends(get_app_service)):
    result = service.claim_import_by_token(payload.token, payload.userId)
    return ApiResponse(
        data={
            "importBatch": result["importBatch"].model_dump(),
            "card": result["card"].model_dump(),
