...
def parse_callback_body(raw_body: bytes, content_type: str, settings: Settings, query: dict[str, str | None]) -> dict:
    ...
    if encrypted:
        ...
        if not verify_signature(settings.wecom_callback_token, timestamp, nonce, encrypted, signature):
            raise WecomCryptoError("企业微信回调签名验证失败")
        decrypted_xml = decrypt_aes_message(settings.wecom_encoding_aes_key, encrypted, settings.wecom_corp_id)
