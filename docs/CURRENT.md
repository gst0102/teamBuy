...
## 147. 2026-09-15 新账号限制截图与新旧设备证据边界

- 用户提供的现场截图显示新账号被限制“所有社交场功能”，页面原因文案为“骚扰/虚假营销/欺诈等违规行为”，并显示 2026-09-29 15:54 后可解除。这是账号级社交功能限制，不是单个群的禁言；截图没有限制开始时间或具体触发动作。
- 17:03 新设备运行的状态文件只有 `reason=queued` 的完成回调；生产只读记录中没有 `android-02` 的群发任务、候选记录或该截图账号的账号记录，也没有收到账号 heartbeat。因此现有证据不能把截图限制归因于本项目这次新设备运行。
- 可确认的测试差异是：旧设备曾用 `TEST_SINGLE`、单一既有账号执行 c1001/act-001 的两群原生转发；新设备配置是 `TEST_ONLY`、两个账号槽位。截图中的新账号不在当前新设备配置和生产任务记录中。两机均为 Android 15、AScript 4.0.12、HID；分辨率/机型差异以及新机缺少若干系统权限只能解释自动化前置不稳定，不能作为微信处罚原因。两机蓝牙权限均未授予。
- “新账号信任度较低，或短时间重复相同卡片/文字向多个群发送”只能作为反滥用风险的有界推断；本次 17:03 运行没有创建发送任务，缺少实际发送行为证据，不能据此下结论。本轮未重发、发测试消息、操作手机或修改运行代码。
