...
- `automation/ascript/__init__.py` 是 AScript 入口；代码入口显示为“微信助手”，提供全部扫描、每日增量扫描、群发批次、设备状态和任务日志；专项规则见 `automation/ascript/AGENTS.override.md`。当前已同步到设备工程 `wechat_assistant`（入口页面显示“微信助手”）；手机实际启动的是哪一个工程，仍必须以设备上的实际工程路径/文件清单和运行日志确认，不能仅凭本地代码或同名项目推断。
