...
## 2026-09-01：确认活码直传创建与“找群 / 我的发布”双 Tab 设计（仅设计，未改业务代码）

- 根据用户反馈，将活码创建原则收敛为“直接上传二维码并生成固定入口”；不要求用户填写名称，名称由系统自动生成。
- 统一生成“找群 / 我的发布”双屏效果图：前者服务于发现并消费公共有效群资源，后者服务于管理自己的发布记录，明确不同数据源和不同操作。
- 复核当前同内容根因：`miniprogram/pages/group-resource-library/index.js` 的本地 `loadState()` 仍把同一份 `decorated` 同时写入 `groups` 和 `myGroups`，且数据来自用户隔离的 `wx.setStorageSync`；正式实现必须接入服务端公共查询和当前用户查询。
- 本轮没有修改业务代码、没有部署生产、没有上传微信体验版；效果图仅用于确认信息架构和视觉方向。
...
## 2026-09-01：微信群工具入口与群资源库规则本地预览

### 本轮完成

- “我的”页的“微信群工具”从占位宫格改为进入独立的微信群工具页；不再显示“即将上线”或“已上线”状态标签。
- 微信群工具页先保留两个核心入口：群资源库、我的活码。群资源库复用旧页面；我的活码仅保留视觉入口，用户端活码接口尚未接入，当前不可操作。
- 群资源库用户可见规则收口为：每天最多新发布 1 个群资源，发布奖励累计最多 100 积分，更新已有群二维码不重复奖励，自然到期不扣分，查看他人二维码消费 30 积分。
- 群资源库本地预览增加每日发布限制和累计奖励上限的本地表现，并删除“持续更新获得更多积分”“失效会扣罚”等旧话术。

### 验证与边界

- 全量小程序 JavaScript `node --check` 通过，79 个 JSON 文件解析通过；工具入口路由检查通过，`git diff --check` 通过。
- 本轮只修改小程序入口、群资源库本地预览逻辑和用户文案，没有修改后端、数据库、生产配置、生产镜像或微信体验版。
- 当前群资源库仍使用 `wx.setStorageSync` 本地数据；每日上限、累计 100 分上限和积分消费都不能作为生产安全边界，正式开放前必须迁移到服务端统一积分账本并增加幂等校验。
- 当前没有自动上传小程序。用户需在微信开发者工具清缓存、重新编译后，从“我的 → 微信群工具”查看新入口；活码要等用户归属接口、二维码上传和到期订阅链路完成后再开放。
...
## 2026-09-01：微信群工具三 Tab 本地重构

### 本轮完成

- 将微信群工具主页面收敛为三个同级 Tab：`找群`、`我的发布`、`我的活码`。找群只读取公共有效资源，我的发布只读取当前用户资源并保留过期记录用于更新，避免两个 Tab 再显示同一份本地列表。
- 找群增加城市、行业、用途和关键词筛选；发布流程只保留城市/全国、行业、用途和 1～7 天有效期等必要配置，移除没有实际决策价值的人数区间和活跃度。过期资源自动从找群列表隐藏。
- 新增群资源服务端模型、JSON/PostgreSQL 仓储表及接口：发布、我的资源、更新二维码、删除、查看二维码。查看他人二维码通过 `mutual_help` 统一积分账户扣 30 分，并以稳定幂等键保证重复查看不重复扣分。
- 实现服务端每日最多新发布 1 个群资源、累计发布奖励最多 100 分；奖励先记为待确认，不在上传图片时直接发放。更新原资源不重复奖励、不占当天新发布次数；删除后当天重新发布仍受每日上限约束。
- “我的活码”接入用户端真实闭环：直接上传可识别的群二维码，自动生成名称和固定入口；更新只替换当前二维码并刷新 7 天有效期，固定入口不变；支持下载活码、删除和所有权校验。
- 保留现有固定入口链路：固定入口继续返回 `200 HTML`，再展示当前二维码图片，没有改回 302。

### 验证与边界

- 后端 `backend/tests/test_group_tools.py` 覆盖公共/私有数据隔离、每日限制、查看扣分幂等、更新/删除权限、活码固定入口和所有权；完整后端测试、Python 编译、小程序 JS/JSON 静态检查和 `git diff --check` 已执行。
- 本轮没有部署服务器、没有执行生产数据库建表、没有修改生产 `.env`、没有创建生产充值订单、没有调用 `test-confirm`，也没有上传微信体验版。
- 活码到期前两天订阅提醒尚未接到定时发送事件；此前配置的“服务到期提醒”模板仍只是配置基础，不能宣称线上提醒已生效。
- 发布奖励审核/释放尚未接入管理员审核动作；当前页面明确显示“待确认”。城市选择当前使用小程序省/市选择器并同时保存城市标签，正式上线前仍需在真机确认城市筛选和生产数据的跨用户可见性。

## 2026-09-01：群工具页面文案收口与活码上传错误处理

### 本轮完成

- 移除群资源库页面会变形的“微信群工具”眉标题；活码 Tab 仍保留“免费工具”识别，群资源库直接突出“群资源库”。
- 发布流程移除没有实际决策价值的“人数区间”和“活跃度”，群资源卡片及发布成功页同步不再展示这些字段；后端字段保留兼容旧数据，不影响已有接口读取。
- 图片选择增加 `chooseMedia` 到 `chooseImage` 的兼容降级，避免部分微信基础库返回 `nofund/not found/not support` 时无法继续选择图片。
- 活码上传/更新遇到 404 或 `nofund/not found` 时显示“活码服务尚未部署，请先部署服务器后再试”，不再把英文底层错误直接暴露给用户。

### 边界

- 当前小程序 `apiBaseUrl` 仍指向生产域名，而本轮新增的用户活码接口尚未部署到生产；因此真机要完成活码上传，还需要后续按腾讯云部署文档部署后端和 `group_resources` 表。
- 本轮没有部署服务器、没有修改生产 `.env`、没有上传微信体验版；`NoFund` 的最终来源仍需在部署后用真机日志确认是选择器兼容还是旧接口 404。

## 2026-09-01：免费工具名称收口并准备部署

### 本轮完成

- 活码 Tab、顶部导航和页面主标题统一改为“免费工具”。
- 删除活码页顶部重复的“免费工具”眉标题，避免同一页面重复展示。
- 本轮准备将此前未上线的群资源与用户活码后端代码部署到生产，保持生产 `.env`、secrets、媒体目录和数据库卷不变。

### 边界

- 小程序前端仍需用户在微信开发者工具中重新编译并手动上传体验版；本轮不自动上传。

### 部署结果

- 生产已切换群资源与用户活码后端热修复镜像，备份目录为 `/home/ubuntu/teambuy-backups/20260901-204149-group-tools-live-qr/`。
- `group_resources` 和 `live_qr_codes` 表已确认存在；API、backend-worker、archive-worker、PostgreSQL 均 healthy，公网 `/health` 通过。
- 未修改生产 `.env`、secrets、媒体目录和数据库卷；未创建生产测试充值订单、未调用 `test-confirm`、未上传微信体验版。

## 2026-09-01：活码提交后消失问题诊断

### 诊断结果

- 生产日志显示 20:52 左右两次 `POST /api/live-qr-codes` 均返回 200，随后活码图片和列表 GET 也返回 200；没有对应的删除请求。
- 当前数据库没有这两次新提交，仍只有 8 月 30 日的旧活码记录，说明记录是在提交成功后被覆盖，不是前端单纯没有渲染。
- 根因高概率是 `get_mutual_help_status()` 把 GET 做成了全量状态保存：读取整份 `AppState` 后调用 `repo.save(state)`；PostgreSQL 全量保存会删除并重写所有表。并发请求使用旧快照时，会把刚写入的活码及媒体记录覆盖掉。
- 前端 `loadLiveQrCodes()` 在列表请求失败时直接设置空数组，会放大“自动消失”的视觉表现，但不是本次 200 提交记录丢失的主因。

### 本轮边界

- 本轮只完成生产日志、数据库和代码路径诊断，没有修改代码、没有创建测试活码、没有再次部署。

## 2026-09-01：21:05 活码再次复现，确认旧快照覆盖链路

### 复现证据

- 用户 21:05 提交对应生产日志时间为 UTC 13:05：`POST /api/live-qr-codes` 在 21:05:22 返回 200，二维码图片在 21:05:22.8 返回 200，说明上传和活码创建接口当时均已成功。
- 随后 `GET /api/scrm/mutual-help` 在 21:05:25 返回 200，紧接着 `GET /api/live-qr-codes` 在 21:05:25 返回 200；数据库复查仍没有该用户的新活码行，只剩 8 月 30 日的旧记录。
- 21:05 时间段没有对应 DELETE 请求。结合 `get_mutual_help_status()` 的 GET 全量保存和 PostgreSQL 全量删除重写逻辑，本次复现进一步确认：提交结果先被页面显示，随后并发/滞后的旧 `AppState` 回写覆盖新活码，列表再次读取为空，于是表现为“显示后消失”。

### 当前边界

- 本轮仍只做生产证据核对和文档记录，没有修改业务代码、没有创建测试活码、没有重新部署。
- 在修复全量快照回写竞态前，不建议继续用真实二维码反复提交；后续应先做行级/事务级保存，再回归验证。

## 2026-09-01：免费工具“我的活码”子页面第一性原理设计

### 设计结论

- 子页面的首要任务是让用户维护一个长期不变的分享入口，而不是浏览历史；因此页面主结构采用“当前活码卡片 + 历史记录”，不把“历史活码”单独做成首层入口。
- 当前活码卡片左右并列展示“微信群码”和“固定活码”，中间用箭头表达绑定关系；同一张卡集中展示“有效期至、剩余天数、到期前提醒、更新二维码”。
- 订阅消息点击后应直接打开对应活码记录并突出“更新二维码”，用户不需要重新寻找上传入口；更新只替换当前微信群码并刷新 7 天有效期，固定入口保持不变。
- 历史记录只展示最近 3 次替换和状态。因为固定活码不变，历史列表不重复展示右侧活码，避免让用户误解为每次更新都生成新链接。

### 本轮边界

- 已生成移动端效果图供产品确认，本轮没有修改小程序业务代码、没有修改后端、没有部署或上传体验版。

## 2026-09-01：微信群工具改为群资源库三 Tab 主页面

### 本轮完成

- 按“找群、管理自己的发布、管理自己的活码”三个用户目标重排信息架构：个人页的微信群工具入口直接进入群资源库，不再增加一个只用于分流的中转页。
- 群资源库主页面新增“找群 / 我的发布 / 我的活码”同级 Tab；找群默认只显示未过期资源，我的发布保留过期资源用于后续更新。
- 群卡片补充“有效至”日期和过期状态；已过期资源不再提供查看二维码操作，当前有效数量单独展示。
- 我的活码先接入页面层级和视觉空态，保持绿色免费工具识别；创建动作明确提示功能尚在准备，不伪造尚未完成的用户端接口。
- 旧 `subpackages/my-tools` 路由保留，但打开后跳转到新的主页面，避免历史路径继续展示重复中转页。

### 边界

- 本轮仍是小程序本地页面调整，没有部署服务器或上传微信体验版。
- 群资源库的跨用户公共资源、服务端审核、统一积分账本和我的活码用户端链路仍未完成；页面上的每日发布、累计奖励和有效期表现不能代替服务端约束。
## 2026-09-01：确认群资源 Tab 根因并完成活码方案设计

### 本轮完成

- 核对“找群”和“我的发布”页面内容相同的根因：`loadState()` 将同一份本地 `decorated` 列表赋给两个数据字段，当前 Tab 没有真正的数据源隔离。
- 明确后续要拆成公共有效群资源与当前用户发布资源两条查询链路；在服务端公共资源未接入前，不用本地复制数据制造“找群”效果。
- 生成活码三状态效果图，补齐无活码时的“上传群二维码”首要入口、上传创建页和已有活码管理页。
- 更新活码创建、二维码校验、固定入口、到期前两天订阅提醒和上传新二维码更新流程图，明确更新只替换二维码、不改变固定入口。

### 边界

- 本轮只做问题确认和设计，不修改业务代码、不部署生产、不上传微信体验版。
- 效果图中的“正常”“有效至”“剩余 2 天提醒更新”是目标状态，不代表用户端活码功能已经实现。

## 2026-09-01：活码历史页面、快照覆盖修复与到期提醒

### 本轮完成

- 新增“我的活码”当前记录与历史更新展示：左右对照“微信群码 / 固定活码”，显示剩余天数、有效期、固定入口和更新/保存/删除操作；更新保留原固定入口并记录最近版本历史。
- 修复互助积分状态读取触发全量 `AppState` 回写的问题：首次创建账户/流水使用行级保存，状态与流水读取不再用旧快照覆盖活码、媒体或其他业务数据。
- 活码列表请求失败时保留最近成功列表，不再用空数组制造“提交后消失”的二次假象。
- 接入用户提供的“服务到期提醒”订阅模板配置；服务端在活码剩余两天内按“记录 + 版本”幂等排队一次，发送后保留明确的“再次开启提醒”入口，并深链到具体活码更新页。
- 管理员更新二维码也会写入活码历史版本，兼容旧记录的默认空历史结构。

### 验证与边界

- `./.venv312/bin/python -m pytest -q backend/tests`：326 passed。
- Python 编译、小程序 JavaScript 语法、`git diff --check` 均通过；全仓测试仍被既有 `platform/wecom-archive-core/tests/test_engine.py` 的 `app.codec` 导入路径问题阻断。
- 小程序不自动上传体验版；本轮后端已部署并追加活码订阅模板配置，但真实微信订阅授权、二维码上传/更新/下载仍需用户手动上传体验版后验收。

## 2026-09-01：活码增加投放样式切换

### 本轮完成

- “我的活码”新增两种投放样式：默认“纯二维码”，以及“保留原图”。
- 选择“保留原图”时，服务端读取用户上传的原图，定位其中二维码，只替换二维码内容；原图中的群头像、群名称、文字和排版保持不变。
- 原始群二维码仍作为当前目标图保存，投放图作为独立派生媒体保存；更新二维码时会按原样式重新生成投放图，固定入口和 `code` 不变。
- 已有活码可以直接切换样式，新建活码可以在上传前选择样式；“保存活码”成功后保留“要开启到期提醒吗？”授权提示，并保留卡片内的“开启提醒”入口。
- 管理员更新已有活码时，如果该活码使用“保留原图”，也会同步重生成投放图。

### 验证与边界

- `./.venv312/bin/python -m pytest -q backend/tests/test_group_tools.py`：4 passed，覆盖创建、切换、回切和生成图中的固定入口二维码可识别。
- 小程序 `node --check`、Python 编译和 `git diff --check` 通过。
- 已按腾讯云部署文档完成后端热修复部署；备份目录为 `/home/ubuntu/teambuy-backups/20260901-230846-live-qr-style/`，三类镜像保留 `rollback-20260901-230846-live-qr-style` 标签。小程序仍未自动上传体验版。
- 部署后 API、backend-worker、archive-worker、PostgreSQL 均 healthy，8004 端口、公网健康检查和 `/api/live-qr-codes/{qr_id}/style` 路由核对通过。

## 2026-09-02：我的活码改为多记录列表与同页详情

### 本轮完成

- 将“免费工具 / 我的活码”改为列表优先：每条记录显示二维码缩略图、群名、备注、生成日期、使用状态和剩余有效期。
- 活码列表始终保留“新增活码”入口；后端原有多记录能力不变，用户可以为不同群或不同投放入口分别保存活码。
- 点击某条记录后，在同一页面展开完整详情，继续显示“微信群码 → 固定活码”、投放样式、有效期、提醒、保存、更新和删除操作。
- 删除用户界面的“历史更新”区块；同一固定入口的版本信息仍由后端保留用于内部版本和提醒幂等，不再作为用户日常操作内容展示。
- 订阅消息深链携带的 `qrId` 可以直接定位到对应活码详情，避免提醒后重新寻找更新入口。

### 验证与边界

- `node --check miniprogram/pages/group-resource-library/index.js` 通过。
- `node --check miniprogram/services/api.js` 通过。
- WXML `view/block/image` 标签数量平衡，`git diff --check` 通过，历史更新文本和前端引用已清除。
- `./.venv312/bin/python -m pytest backend/tests/test_group_tools.py backend/tests/test_live_qr.py -q`：12 passed。
- 本轮只修改本地小程序页面，没有修改后端、没有部署生产、没有自动上传微信体验版；当前线上版本保持上一轮活码投放样式部署结果。

### 部署核对补充

- 用户要求部署服务器后，核对确认本地后端与线上容器源码哈希一致；线上 API、两个 worker、PostgreSQL 和公网健康检查均正常。
- 本轮只有小程序前端变更，不需要重复构建或重启后端；小程序仍需通过微信开发者工具手动上传体验版生效。

## 2026-09-02：收窄新增活码按钮

- 修复小程序原生 `button` 宽度规则把“新增活码”撑满的问题。
- 说明卡保持单行横向布局，按钮明确固定为 `160rpx`，左侧说明文字获得稳定空间并减少不必要换行。

## 2026-09-02：互帮互助双积分账本开发、审查与生产部署

### 本轮完成

- 互帮互助积分拆为两个独立账本：`基础积分`（首次进入赠送 100 分、基础任务）和 `奖励积分`（奖励积分档位/支付入账、奖励任务）。两者各自维护余额、累计发放、累计消耗和流水；页面总积分仅作为展示汇总，不参与扣减。
- 旧版单余额数据兼容迁移：历史 `recharge` 流水归入奖励账本，其余历史流水归入基础账本；旧总余额不会凭空生成奖励积分。内存 JSON 和 PostgreSQL 都支持迁移。
- 积分幂等键增加 `pointType` 分区，避免两个账本相同业务键互相冲突；普通任务、本地小程序任务、羊毛解锁/打赏均沿用任务选择的积分类型。群资源查看固定消耗基础积分，接口返回双账本总览，避免页面把基础余额暂时显示成总余额。
- 生产端订单记录补充 `point_type` 字段；历史 API、数据库表和内部方法保留旧命名以兼容已有客户端和运营数据，但用户端和运营页显示使用“基础积分 / 奖励积分 / 获取奖励积分”。

### 验证

- 定向积分测试：8 passed。
- 后端全量：`./.venv312/bin/python -m pytest -q backend/tests`，330 passed。
- 小程序修改后的 JavaScript `node --check`、SVG XML 校验、`git diff --check` 通过；本地双账本脚本验证了初始化 100/0、分别扣增、旧版余额迁移到基础账本。
- 全仓默认 pytest 仍有既有路径问题：`platform/wecom-archive-core/tests/test_engine.py` 导入 `app.codec` 失败；不属于本轮改动。

### 生产部署

- 已按腾讯云热修复流程备份并部署 API、backend-worker、archive-worker；备份目录：`/home/ubuntu/teambuy-backups/20260902-015339-dual-ledger/`。
- 保留旧镜像回滚标签：`rollback-20260902-015339-dual-ledger`；未修改生产 `backend/.env`、secrets、媒体目录或 PostgreSQL 数据卷，未创建生产测试订单，未调用测试支付确认。
- 生产 PostgreSQL 已核对 `point_type` 列和双账本唯一索引；三类服务、PostgreSQL、`127.0.0.1:8004`、公网 `/health` 和 `/health/db` 均通过。
- 小程序没有自动上传体验版；前端双账本页面仍需用户在微信开发者工具清缓存、重新编译并手动上传后才会生效。

## 2026-09-02：群资源库返回入口与积分展示稿

### 本轮完成

- 群资源库主页面启用统一的自定义导航返回箭头；发布流程继续使用原有流程内返回，避免出现重复返回层级。
- “我的工具”中的群资源库入口增加明确的蓝色资源工具识别、浅蓝图标背景和可用状态视觉，不再使用容易被误解为不可用的灰色样式。
- 生成一张群资源库积分展示效果稿：以“可用积分”为主值，下方区分“基础积分 / 奖励积分”，并将群工具的扣增规则压缩为用户可理解的一行说明。

### 边界与验证

- 本轮没有修改积分计算、账本扣增、群资源状态或 `-30/+20` 业务规则；效果稿中的 `-15/+10` 只是待确认设计，不代表当前页面已切换。
- 小程序 JavaScript `node --check`、页面 JSON 解析和 `git diff --check` 均通过。
- 仅修改本地小程序 WXML/WXSS；未部署服务器，未上传微信体验版，需用户在微信开发者工具清缓存、重新编译后人工确认返回箭头和蓝色入口视觉。

## 2026-09-02：群资源库积分文案与二维码保存

### 本轮完成

- 群资源规则与后端实际计费统一为：查看他人群二维码扣除 15 个基础积分；新发布有效群奖励 10 个积分，累计奖励上限仍为 100 分。
- 群资源页新增共享双账本摘要：总值显示“可用积分”，基础积分始终显示，奖励积分余额为 0 时隐藏，余额大于 0 时显示。
- 查看二维码弹窗按钮由“知道了”改为“保存”，执行下载当前二维码并保存到相册；保存成功后关闭弹窗，失败时保留弹窗并提示授权或下载问题。

### 验证与边界

- .venv312/bin/python -m pytest backend/tests/test_group_tools.py -q：4 passed。
- node --check miniprogram/pages/group-resource-library/index.js、页面 JSON 解析和 git diff --check 通过。
- 未部署服务器，未上传微信体验版；小程序需在微信开发者工具清缓存、重新编译后人工确认积分行隐藏、弹窗居中、相册保存权限和真实二维码保存结果。
- 发布奖励仍按现有流程先进入“待确认”，本轮只同步奖励金额，不新增管理员审核/投诉处理流程。

## 2026-09-02：群资源库入口与积分视觉修正

### 本轮完成

- 积分卡改为更深的蓝色层级：强化卡片边界、标题、拆分项和规则文案的对比度，保留“可用积分 / 基础积分 / 奖励积分”的信息结构；奖励积分为 0 时继续隐藏。
- 二维码弹窗“保存”按钮使用更高优先级的固定宽度、无默认内边距和 flex 居中规则，避免原生 button 样式导致按钮文字偏移。
- “我的”页的“微信群工具”入口改为蓝色可用态，并增加轻量蓝色阴影，不再使用灰色不可用视觉。
- 自定义导航新增可选返回兜底地址；群资源库从“我的”进入或无页面栈时，顶部箭头可以返回“我的”，有页面栈时仍优先使用正常返回。

### 验证与边界

- `node --check miniprogram/components/custom-nav/index.js`、`node --check miniprogram/pages/group-resource-library/index.js`、页面 JSON 解析和 `git diff --check` 通过。
- 本轮仅修改本地小程序视觉与导航兜底，没有部署服务器，也没有上传微信体验版；需在微信开发者工具清缓存并重新编译后人工确认真机效果。

## 2026-09-02：微信群工具按需登录与入口间距

### 本轮完成

- “我的工具”宫格入口的图标与文字间距由 `14rpx` 调整为 `18rpx`，让两个元素分组更清楚。
- 群资源库页面不再在 `onShow` 时强制跳转登录，未登录用户可以先浏览公开的“找群”页面，与互帮互助首页行为一致。
- 未登录时不再请求需要 `ownerUserId` 的“我的发布”和积分接口；点击“我的发布”、发布群资源、活码、查看二维码等账号动作时，才带返回地址进入登录页。

### 根因

- 页面路由和 `app.json` 配置本身存在且有效；打不开的实际原因是群资源库 `onShow()` 对空用户直接执行 `wx.reLaunch({ url: "/pages/login/index" })`，而互帮互助首页允许未登录浏览，因此两者表现不一致。

### 验证与边界

- 需要在微信开发者工具清缓存并重新编译后确认：未登录可进入微信群工具、受保护动作可回到对应 tab、图标和文字间距符合预期。

## 2026-09-02：群资源发布状态统一显示为“审核中”

### 本轮完成

- 后端群资源 payload 的用户展示字段由“待确认”改为“审核中”。
- 小程序列表、发布成功页、发布流程提示统一使用“审核中”；内部 `rewardState: "pending"` 保留不变，继续供审核和奖励结算逻辑判断。
- 前端额外兼容旧接口仍返回“待确认”的情况，收到旧文案时也会归一化显示为“审核中”，避免线上后端尚未更新时页面继续出现旧词。

### 对抗式检查

- 检查了后端状态字段、前端状态装饰、我的发布列表、发布成功页和发布流程提示，确认没有遗漏群资源用户展示文案。
- 投诉、自动暂停、退款、追回奖励和管理员处罚仍未在本次文案修复中新增；它们属于后续业务闭环，不能因为展示为“审核中”就误认为已经实现。

## 2026-09-02：PC 业务工作台新增“微信群”审核子工作台

### 本轮完成

- 在 PC 运营后台“业务工作台”下增加“微信群”子入口，队列读取真实用户发布的 `group_resources`，与管理员手工群二维码录入列表分离。
- 增加状态统计、状态/关键词/城市/行业/投诉筛选、分页、二维码详情、投诉记录、查看扣分流水、奖励流水和审核日志。
- 增加确认有效、驳回、暂停/恢复、标记无效、退回查看积分、额外处罚和永久下架操作；确认有效奖励 +10 进入奖励账本且幂等。
- 增加投诉接口：同一二维码版本由两个不同且实际付费查看的用户投诉时自动暂停并逐笔退款；重复投诉和重复退款受幂等保护。
- 余额不足的追回/处罚记录为处罚欠分并暂停发布，不产生负积分。
- 补充 PC 审核工作台测试清单和 4 个后端回归测试。

### 验证与上线边界

- `pytest backend/tests/test_group_tools.py backend/tests/test_group_resource_review_workbench.py -q`：9 passed。
- `pytest backend/tests/test_app.py -q`：185 passed。
- 运营后台内联脚本 `new Function` 解析、后端 Python 编译和 `git diff --check` 通过。
- 本轮未部署服务器、未修改生产数据、未上传小程序体验版；需先在本地或测试环境用管理员 Token 人工点验 PC 工作台，再单独安排上线。

### 对抗式审查补充

- 用户查看与投诉/审核动作共用积分锁，避免资源刚被自动暂停后仍发生一次扣分。
- 处罚前补齐发布者的共享基础/奖励积分账户，确保尚未打开互助页的用户仍按初始基础积分 100 分计算。
- 已永久下架资源在接口层拒绝再次审核、无效处理或处罚；重复无效处理保持幂等。

## 2026-09-02：微信群审核工作台生产部署

### 部署结果

- 已将本轮后端依赖链和 PC 运营后台部署到 `81.70.84.35:/home/ubuntu/teamBuy`，未覆盖生产 `.env`、密钥、媒体目录或数据库卷。
- 生产 API、OCR worker、会话存档 worker 均已切换到部署镜像；Compose 保持 `8004 -> 8000` 端口映射。
- 生产备份目录：`/home/ubuntu/teambuy-backups/20260902-063609`；二次权限修复备份目录：`/home/ubuntu/teambuy-backups/20260902-064326`。

### 线上验证

- API、两个 worker 和 PostgreSQL 均为 healthy，公网 `https://teambuy.lifelove.top/health` 返回 200，最近 3 分钟容器日志未发现错误关键词。
- 公开 `GET /api/group-resources` 返回 200；未登录发布、查看、投诉均返回 401；管理员审核队列和统计接口鉴权后返回 200。
- PC `/ops` 页面已包含“微信群”子工作台。
- 小程序代码未通过服务器发布；仍需在微信开发者工具重新编译并手动上传体验版。

## 2026-09-02：我的工具入口间距微调

- 个人页“我的工具”两个入口的图标与标题间距统一为 `28rpx`，改善“互帮互助”和“微信群工具”的视觉呼吸感。
- 本轮仅修改小程序本地 WXSS，未部署服务器；仍需在微信开发者工具重新编译查看。

## 2026-09-02：商机合作高保真实现、自检与对抗式审查

### 本轮完成

- “我的工具”增加“商机合作”入口；无名片时进入电子名片编辑，有名片时进入商机合作页。
- 新增高保真商机合作页和客户详情页，包含动态合作、合作能力、我的名片三个 Tab，行业/细分行业筛选，搜索，服务端分页懒加载，图片懒加载，空态和错误态。
- 电子名片编辑页增加商机合作标签区：行业、细分行业、行业标签、当前合作意向和有效期；“我能提供”成为至少一项的必填能力字段。
- 新增商机名片列表、详情和联系方式解锁 API；详情默认脱敏，联系方式解锁消耗 1 个基础资源积分，同一目标 24 小时幂等，本人免扣。
- 修复生产公共接口身份边界：列表/详情 GET 可匿名浏览，`viewerUserId` 不再被生产接口信任，解锁 POST 保持身份保护。

### 验证

- 商机相关回归集：14 passed。
- 完整 `backend/tests/test_app.py`：185 passed，1 个既有 JSON/Postgres 环境断言失败。
- Node JS 检查、JSON 校验、Python 编译和 `git diff --check` 通过。
- 已完成一次对抗式审查，修复联系方式越权和过期动态意向问题；详细结论见 `docs/qa/商机合作_对抗式审查报告.md`。

### 上线边界

- 本轮未部署服务器，未上传小程序体验版。
- 需在微信开发者工具清缓存、重新编译并人工检查高保真页面、懒加载、积分确认和真机按钮/标签排版。

## 2026-09-02：商机合作文案与联系方式交互微调

- 商机合作第一 Tab 由“动态合作”改为更直白的“找合作”，副标题、结果标题和空态同步调整；顶部改为“找对合作伙伴 / 让合作更简单”。
- Tab 主文字、辅助文字、行业筛选、细分筛选和列表标签字号按真机截图放大；详情页能力标签同步放大。
- 本人“客户页预览”增加说明：当前是发布者本人视角，外部客户打开后才显示沟通动作；商机详情解锁后手机号增加“拨打”和“复制”两个操作。

### 验证补充

- 最新商机相关回归集：15 passed；JS、JSON、Python 编译及 `git diff --check` 继续通过。

## 2026-09-02：商机合作 Banner 按参考稿重排

- 将商机合作 Banner 从“悬浮积分卡”调整为更接近参考稿的信息层级：`BUSINESS CONNECTION`、核心主张“让对的人相遇，让合作自然发生。”、说明“从一张名片开始，看见能力，也表达你的需求。”、基础积分独立信息行和合作风险提示。
- 保留现有蓝绿渐变品牌背景，只借鉴参考稿的文字组织、分隔线和信息节奏，避免页面与现有商机合作 Tab/筛选区视觉割裂。
- 风险提示使用独立提示符号和可换行正文；积分数字、说明文字及提示文案继续使用 `rpx`，不阻塞列表懒加载。

### 验证

- 商机相关回归集：15 passed。
- 商机页 JS 检查、JSON 校验、后端相关文件 Python 编译和 `git diff --check` 通过。
- 本轮未部署服务器，未上传小程序体验版；需在微信开发者工具重新编译后人工确认 Banner 在不同屏幕宽度下的换行和纵向占高。

## 2026-09-02：商机合作 Banner 辅助提示与 Tab 对比度微调

- 按真机截图反馈，将 Banner 底部“温馨提示”收窄到约原内容宽度的一半并降低字号，使其明确保持辅助信息定位，不与主标题、积分信息竞争视觉焦点。
- 将“找合作 / 合作能力 / 我的名片”及其副标题的非选中态文字改为更深的灰色；选中态继续使用品牌青绿色，保持当前 Tab 的状态识别。

### 验证

- 本轮仅调整小程序 WXSS；未改动商机合作数据、积分、联系方式解锁或懒加载逻辑。
- 需在微信开发者工具重新编译后人工确认窄屏下提示文案仍可读、Tab 三列文字不换行。

## 2026-09-02：修正商机合作 Banner 提示区的调整方向

- 根据真机反馈确认上一轮把提示区收窄成 `56%` 是对“宽度”的误解；本轮恢复提示区原有横向宽度。
- 通过缩短提示分隔线间距、将“温馨提示”和风险正文改为同一行优先排版、压缩行距，降低提示区纵向占高；窄屏仍允许正文自然换行。

## 2026-09-02：移除 Banner 第三段的蓝色背景与多余留白

- 根据最新截图确认，风险提示不应继续属于蓝色 Banner 的第三段；已将“温馨提示”移到 Banner 外的页面浅色背景中，保持全宽但不再占用蓝色区域。
- 蓝色 Banner 只保留核心说明和基础积分，并移除 `min-height:500rpx` 固定高度，让容器随内容收缩，消除积分下方的大块空白。
- 风险提示改为轻量横向提醒，仍保留小屏自然换行；核心信息与辅助提醒在视觉和结构上完成分层。

## 2026-09-02：完成“我的工具”三功能系统性对抗式审查

- 确认商机合作详情页解锁后支持手机号“拨打/复制”、文字微信复制、微信二维码查看；普通公开客户页是另一条链路，本人预览隐藏外部沟通操作，其他用户通过客户沟通组件操作。
- 发现商机列表在首屏请求竞争时，快速切换 Tab/行业/搜索会被 loading guard 忽略，存在“标题已切换但列表仍是旧条件”的 P1 问题。
- 发现互帮互助任务、提交和结算仍主要由小程序本地 storage 驱动，服务端只有积分状态/充值/行为记录；跨用户跨设备的正式任务闭环尚未成立。
- 发现微信群投诉接口虽已在后端实现，但小程序没有用户投诉入口；二维码更新后的是否重新审核需要补充明确规则。
- 发现商机市场付费解锁与既有公开客户页之间存在收费边界，当前按既有决策保留，但必须在上线验收时确认是否允许公开分享绕过 1 积分门槛。
- 审查报告保存为 `docs/qa/我的工具三功能_系统性对抗式审查报告.md`。

### 验证

- 群资源、活码、审核、投诉保护和双账本回归：14 passed；商机相关回归合计 15 passed。
- 相关小程序 JS、`app.json` JSON 解析和 `git diff --check` 通过。
- 本轮未修改业务代码、未部署服务器、未上传小程序体验版。

## 2026-09-02：我的工具三功能对抗式审查修复

- 互帮互助任务、提交、验收和结算接入服务端实体；以 `openid` 映射的 `userId` 作为跨设备业务身份，服务端双账本结算为真源，本地数据仅作迁移/缓存降级。
- 商机合作补齐服务端分页、触底懒加载和请求序号；修复快速切换 Tab、行业、细分行业、搜索时旧响应覆盖新条件的问题。解锁记录在详情重进时复用，不重复扣费。
- 商机联系方式增加“联系方式失效？举报”；同一用户同一名片版本幂等，并在列表/详情展示风险提醒。公开客户页、正文/摘要/简介/内容块和旧分享快照均不能绕过 1 基础积分联系方式闸门。
- 微信群公共目录改为游标分页和触底懒加载，筛选请求增加请求序号；二维码查看弹层增加“二维码失效？举报”。二维码变化后回到审核中，沿用两名不同付费用户投诉后暂停展示并退款的服务端规则。
- 补充互帮互助表结构、索引和双账本迁移时旧唯一索引清理，避免生产数据库已有索引阻断新账户写入。

### 验证

- 相关后端回归：`202 passed, 1 deselected`；健康检查：`1 passed, 185 deselected`。
- `compileall`、全部小程序 JS `node --check`、小程序 JSON 解析、`git diff --check` 均通过。
- 本轮未部署服务器，未重新编译/上传小程序；生产数据库迁移和微信开发者工具人工验收仍需用户执行。

## 2026-09-02 最新交接：统一积分账户，暂不增加第四个工具入口

### 已完成

- 三个工具共用同一个 `mutual_help` 积分账户；账户内部保留两个账本：基础积分和充值积分。基础积分包含每个用户首次获得的 100 分及所有平台发放的积分，充值积分只保留充值/未来提现对应的积分。
- 所有平台消费统一执行“基础积分优先，不足时再消费充值积分”；群资源查看、商机联系方式解锁、互帮互助任务和羊毛工具均不再各自维护独立可用余额。
- 历史平台奖励只在流水可识别时一次性从旧奖励账本迁入基础积分；旧资源钱包只迁移相对初始 100 分的差额。两类迁移均有幂等标记，不按当前混合余额盲目拆分，避免把充值积分迁错。
- 小程序本地旧 v2 混合余额只按兼容规则归入基础积分；已经有 v3 双账本形状的本地缓存保留充值积分桶，等待服务端流水校正，避免本地无法识别来源时误把充值积分变成平台积分。
- 本轮不新增“积分账户”页面或第四个工具入口，继续保持“我的工具”现有三个功能；互帮互助的“我的”保留积分展示和任务管理。充值入口暂时隐藏，服务端充值数据与能力保留给后续积分中心使用。

### 验证与上线边界

- 后端测试：`344 passed`；定向覆盖平台奖励迁移、充值积分不被迁移、旧资源钱包不重复发放初始积分、基础积分优先消费和重复消费幂等。
- Python 编译、全部小程序 JS `node --check`、小程序 JSON 解析和 `git diff --check` 通过。
- 本轮未部署生产服务器，未上传小程序；生产数据库迁移和小程序重新编译/人工验收仍需按项目上线流程执行。

## 2026-09-02：互助任务分享卡与完成记录实现

- 互助任务分享图升级为 `mutual_task_share_v3`：固定 5:4 文字优先卡片，展示“完成任务赚积分”、三步完成路径、奖励、无需加微信和平台记录/结算说明；任务图片不再阻塞或替代这张转化卡。原生分享标题同步突出完成后获得基础积分，旧快照会因样式版本变化自动失效并重新生成。
- 小程序任务发布增加必填“完成标准”；普通任务统一将原“验收标准”改为“完成标准”。服务端对所有非羊毛任务强制要求至少一段有效文字或图片标准，不能只填标题和入口。
- 任务管理页保留全量提交历史，不再只显示待验收记录；新增已参与、已提交、已验收、已结算四项统计，并展示执行者昵称和记录状态。
- 目标小程序打开成功、返回、用户取消或打开失败写入服务端行为事件，事件带任务入口、会话和受限元数据；统计按任务隔离、排除发布者，并由生产认证身份覆盖前端 userId。

### 本轮验证

- 定向后端回归：`11 passed`（跨设备互助、行为事件、任务级统计隔离、提交/验收/结算和积分结果）。
- 相关小程序 JS `node --check`、Python `compileall`、分享源字段断言和 `git diff --check` 通过。
- 未部署生产后端，未重新编译/上传小程序；微信开发者工具中仍需人工验证分享图实际渲染、原生分享回调和跳转目标小程序后的打开/返回/取消事件。

## 2026-09-03：互助任务分享卡与评论体验补齐

- 任务分享卡升级为 `mutual_task_share_v4`，有真实任务图片时保留图片，无图时使用信息视觉区；统一突出“完成任务得分/查看福利”和“免加微信，选任务（或福利）就能互助”，移除剩余积分、基础积分等干扰信息。
- 三类任务详情均提供评论区，用户可以在决定是否参与前查看评论；列表增加“评论数量 + 值得做比例”，比例分母只包含“值得做”和“不建议”评价。
- 评论的“已完成体验”由服务端根据提交状态判定，覆盖已提交、已验收和已完成；本地演示路径也补充了 `submitted` 状态。
- 新增后端评论汇总和完成标记回归用例，并修复无图分享卡抽象信息区的高度越界风险。

### 本轮验证

- `./.venv312/bin/pytest -q backend/tests/test_my_tools_cross_device_and_gates.py`：4 passed。
- 相关小程序 JS `node --check`、分享源和评论汇总 smoke check、`git diff --check` 通过。
- 本轮未部署服务器，未重新编译/上传小程序；微信开发者工具真机渲染和原生分享仍需人工验收。

## 2026-09-03：按原生分享卡反馈收窄任务卡信息层

- 任务分享样式版本升级为 `mutual_task_share_v5`，将“免加微信，选任务就能互助”作为任务名称下方的副标题，字号提高到 28；移除绿色宣传条、重复步骤和顶部重复奖励。
- 有图任务保留真实图片，无图任务保留轻量信息视觉区；自定义图只保留一个奖励胶囊，微信发送外壳继续由原生分享卡提供。
- 本轮仅调整互助任务分享图渲染，未改变评论、任务结算或其他工具。

## 2026-09-03：互助任务分享卡改为固定 5:4 有图/无图模板

### 本轮完成

- `miniprogram/utils/business-card-share.js` 将互助任务统一为固定 `750×600` 画布：上方为任务视觉区，下方为固定核心卖点条。
- 有图任务保留真实图片并使用完整收纳模式；无图任务显示“互助”信息视觉区、任务标题和摘要，不再使用空白骨架。
- 自定义缩略图移除重复的应用名称、任务标题大段重复和“点击开始任务”按钮，微信原生外层继续负责标题和点击入口。
- `miniprogram/subpackages/my-tools-mutual-help/task-share.js` 将样式版本升级为 `mutual_task_share_v6`，旧快照会自动失效；普通/小程序任务和羊毛福利继续使用不同真实卖点文案。

### 验证结果

- 全部小程序 JavaScript `node --check`：通过。
- `./.venv312/bin/pytest -q backend/tests/test_my_tools_cross_device_and_gates.py`：4 passed。
- 普通任务/羊毛福利分享源、模板类型、5:4 比例和 v6 快照版本 smoke check：通过。
- `git diff --check`：通过。

### 本轮边界

- 固定模板只降低绘制复杂度；首次生成仍包含真实图片下载、Canvas 导出和后端上传，快照就绪后重复分享才会快速读取。
- 本轮未部署生产服务器，未重新编译或上传微信小程序；仍需真机检查有图/无图显示、图片完整性和原生分享回调。

## 2026-09-03：电子名片同步商机并改为双公开选择

### 本轮完成

- 名片编辑页把商机合作改为两个独立开关：`展示我的合作资源` 和 `发布当前合作需求`；名片字段仍只维护一份，保存时自动同步到商机合作草稿。
- 新名片首次默认两项关闭。首次发布完成后显示“名片已完成 / 要不要把你的信息展示到商机合作？”，支持分别公开资源、公开需求或先不公开；选择结果写入 `visibilityChoiceVersion`，后续编辑保留。
- 后端列表、详情、普通公开资料和发布安全校验均支持独立资源/需求公开状态；旧名片继续兼容旧 `enabled/discoverable`，无商机字段的名片不进入公开商机列表。
- 商机合作 Tab 文案由“找合作 / 合作能力”改为“合作需求 / 合作资源”，保留“正在寻找 / 我能提供 / 管理”辅助语义。
- 增加独立公开状态回归测试，覆盖资源-only、需求-only、两项均关闭以及需求隐私不透传。
...
- 未部署生产服务器，未重新编译或上传微信体验版；用户仍需在微信开发者工具中人工确认首次发布后的提示、两个开关的保存回填、列表收录和关闭需求后的隐私显示。

## 2026-09-03：名片发送语义与全量字号对抗式审查

### 审查结果

- 确认名片编辑页“发客户”当前会先 `handleSave()`、再 `publishNote()`，然后进入资料库；真正的 `open-type="share"` 微信原生分享仍需用户再次点击。未把异步保存误实现为自动弹出分享面板。
- 全量扫描 79 个 WXSS、77 个 WXML 和 81 个 JSON；发现 115 个字号较小且伴随固定宽度/不换行的潜在紧凑规则，重点风险是名片编辑页 64rpx 操作按钮和商机详情页 68rpx 联系操作按钮。
- 结论是不做全局机械放大，后续按正文、字段、输入、主要按钮、标签状态、窄操作区分层处理，并在微信开发者工具人工验收。

### 验证

- 全部小程序 JS `node --check`、小程序 JSON 解析、Python `compileall`、`git diff --check`：通过。
- `./.venv312/bin/pytest -q backend/tests/test_my_tools_cross_device_and_gates.py`：5 passed。
- 本轮未部署生产服务器，未重新编译或上传微信体验版。

### 详细报告

- `docs/qa/商机合作与小程序字号_对抗式审查报告.md`

## 2026-09-03：名片编辑与我的工具字号调整

### 本轮完成

- 名片编辑页按语义放大区块标题、说明、字段标签、输入内容、商机合作开关、联系方式选择器、精选资料和底部操作；原 64rpx 的头像/二维码操作按钮同步扩大到 88/84rpx。
- 名片底部“预览客户页”改为“保存并预览”，仍由原有保存逻辑执行后再进入预览；“发客户”继续保持先保存、再发布的安全流程。
- 互帮互助共享样式覆盖任务大厅、任务详情、发布任务、我的任务、任务管理、评论和羊毛福利相关页面；充值页单独调整。
- 微信群工具覆盖浏览、我的发布、免费工具、活码列表/详情、二维码发布流程、举报和弹窗；同步扩大搜索、筛选、发布和二维码操作区。
- 商机合作列表和详情按阅读层级放大，详情联系人操作区扩大到 184rpx，拨打/复制/查看按钮扩大到 86rpx；工具入口页同步调整。
- 未改动 Canvas 分享图字号，也未将全局所有小字号机械替换为同一尺寸。
...
- `./.venv312/bin/pytest -q backend/tests/test_my_tools_cross_device_and_gates.py`：5 passed。
- `git diff --check`：通过。
- 针对 7 个目标 WXSS 的括号平衡、固定窄控件和核心中文按钮宽度估算：通过；排除符号类图标后无大字号窄控件命中。

### 本轮边界

- 未部署生产服务器，未重新编译或上传微信体验版。
- 仍需用户在微信开发者工具清缓存后，按普通小屏、常规手机和大屏/iPad 检查真实中文字体下的换行、按钮居中、双列字段和活码流程。

## 2026-09-03：群工具白屏与充值积分显示核对

### 本轮结论与修复

- 群资源库是主包页面，原脚本直接引用 `subpackages/my-tools-mutual-help/shared.js`，形成主包依赖分包的非法依赖边界，微信端进入后可能表现为白屏。新增主包公共模块 `miniprogram/utils/shared-points.js`，互帮互助分包和群资源库统一复用同一份双账本缓存/同步实现。
- 充值页原来展示兼容字段 `account.balance`，该字段代表基础积分与充值积分的合计，不适合作为“充值积分”余额。现在页面明确展示 `balances.reward`，并说明任务/平台奖励进入基础积分、充值后才进入充值积分账户。
- 服务端核对结果：互助任务结算、群资源发布奖励、羊毛打赏均写入 `base`；充值订单写入 `reward`；消费按基础优先、充值其次。没有发现本轮服务端任务奖励串入充值账本的问题。

### 本轮验证

- 相关后端积分回归：`17 passed`。
- 主包群资源库脚本在 Node stub 环境成功注册；同时确认互帮互助分包复用主包积分模块。
- 相关小程序 JavaScript `node --check`、页面 JSON 解析、`git diff --check`：通过。

### 本轮边界

- 未部署生产服务器，未重新编译或上传微信体验版。
- 用户需在微信开发者工具清缓存后验证“我的 → 微信群工具”进入页面；若旧设备仍显示充值积分 20，需在页面成功请求服务端状态后观察双账本是否被权威数据覆盖，并结合积分流水确认该 20 分的来源。

## 2026-09-03：底部四个 Tab 页面字号调整

### 本轮结论与修改

- 首页、资料、客户雷达、我的四个底部 Tab 采用页面级字号覆盖，没有修改全局 `app.wxss`，避免影响尚未验收的其他页面。
- 首页重点放大反馈卡、最近资料、客户优先处理和资料助手弹窗；资料页重点放大搜索、筛选、资料卡元信息、空态和固定操作；雷达重点放大客户信号、指标、状态、证据和处理按钮；我的页重点放大资产、三个工具入口、浏览记录、客户信号和设置文字。
- 资料卡右侧操作列由 104rpx 调整为 120rpx，并保留按钮省略保护，确保“已停止分享”等动态状态在字号放大后仍有稳定空间。
- 首页主操作按钮改为文字与箭头整体居中；四列雷达指标、四列雷达 Tab 和我的三列工具均保留 `min-width: 0`/`nowrap` 等原有防变形约束。
...
- 81 个小程序 JSON：解析通过。
- 后端 `compileall`：通过。
- 四个目标 WXSS 括号平衡、字号范围、固定控件和动态中文宽度估算：通过。
- `git diff --check`：通过。

### 本轮边界

- 未部署生产服务器，未重新编译或上传微信体验版。
- 仍需用户在微信开发者工具清缓存后，检查四个底部 Tab 在小屏、普通手机和大屏/iPad 下的动态中文换行、按钮居中、资料卡操作列和页面纵向密度。

## 2026-09-03：其余页面字号专项调整

### 本轮完成

- 资料与编辑链路已覆盖资料列表、资料预览、合集列表/查看/分享，以及资源创建、文章编辑、链接确认、卡片编辑、商品/房源/服务编辑、同款复用、后续动作和分享海报页面。
- 客户与系统链路已覆盖客户列表/详情、线索、企业资源搜索、消息列表/会话、导入/认领、登录、会员、反馈、浏览记录、协议和企业微信绑定页面。
- 商机及业务扩展已覆盖商机市场/雷达/详情/收藏/订阅、供需发布与管理、回应包、订单、推广中心、标签和专题，以及经营看板、资料运营分析页面。
- 调整采用页面局部语义覆盖：标题、正文、字段输入、辅助信息、状态标签和操作按钮分别定级；没有做全局机械替换，也没有改接口、积分或业务流程。
- 外部 `web-view`、只负责跳转且不渲染业务文字的页面没有添加无效字号规则；互帮互助页面继续由共享 `shared.wxss` 统一覆盖，隐私页通过协议样式导入获得同一调整。

### 本轮验证

- 109 个小程序 JavaScript `node --check`：通过。
- 81 个小程序 JSON：解析通过。
- 后端 `compileall`：通过。
- 全量 WXSS 花括号检查：通过；新增字号块与页面根节点匹配，未发现大字号与固定宽度控件同规则命中，按钮/操作区保留 flex 居中约束。
- `git diff --check`：通过。

### 本轮边界

- 未部署生产服务器，未重新编译或上传微信体验版。
- 仍需用户在微信开发者工具清缓存后，按小屏、常规手机和大屏/iPad 检查长标题、动态中文、双列表单、固定底部操作和真实系统字体；原生底部 Tab 栏字号仍由微信控制。

## 2026-09-03：全项目用户体验、性能与对抗式审查

### 本轮范围

- 未修改业务代码，按第一性原理检查“打开 → 看懂状态 → 完成动作 → 得到反馈 → 失败可恢复”的完整用户闭环。
- 横向审查小程序启动/页面生命周期、请求与缓存、媒体下载、任务同步、列表分页、错误态、登录、分享、积分、联系方式隐私和生产配置边界。
- 对冷启动、弱网、接口空数组/500、旧数据迁移、大列表、快速筛选和分享图失败等场景做了对抗式检查。

### 已确认的主要风险

- 公开资料详情等待图片下载和本地缓存后才设置首屏视图。
- 互帮互助大厅在渲染前先做本地任务迁移和最多 20 页的服务端同步。
- 群资源库将加载中、失败和真实空列表混为同一空态，筛选刷新还会先清空旧数据。
- 旧商机市场/雷达/收藏/详情页面在空结果或异常时展示未标识的逼真示例数据，部分示例操作有成功反馈。
- 旧资料、订单、供需和商机接口仍有全量返回或全量扫描，数据规模增长后会放大响应和首屏成本。
...
- 全量小程序 JS 检查、JSON 解析、Python compileall、`git diff --check`：通过。
- 根目录直接运行 pytest 的收集导入路径问题已识别；使用正确 PYTHONPATH 后业务测试全部通过。

### 交接边界

- 本轮未部署生产服务器，未重新编译或上传微信体验版。
- 详细报告：`docs/qa/全项目用户体验与性能_对抗式审查报告.md`。
- 下一轮应优先处理示例数据真实性、首屏阻塞和加载/失败/空态区分，再做微信开发者工具人工验收。

## 2026-09-03：全项目体验审查建议统一收口

### 本轮完成

- 移除旧商机市场、雷达、已保存、详情和回应包页面的未标识 mock fallback；空结果显示真实空态，接口失败显示错误态和重试，不再对示例实体反馈保存、跟进或回应包成功。
- 公开资料详情和首页待认领导入改为先返回元数据，图片媒体在后台预热；互帮互助大厅先渲染本地快照，再后台同步服务端任务，并保留同步失败提示。
- 资料、订单、群资源、供需广场、已保存商机和正式商机页面补齐 loading、真实 empty、error、retry 及旧数据保留逻辑；刷新筛选时不再先清空可用列表。
- 资料、订单、旧商机、供需接口增加分页边界；资料库和订单页接入触底加载、请求序号保护和重复项去重；首页订单请求使用 summaryOnly 轻量摘要。
- 新增 `backend/tests/test_core_list_pagination.py` 覆盖资料分页、商机/供需分页响应和订单摘要响应。

### 验证与边界

- `PYTHONPATH=.:backend ./.venv312/bin/pytest -q backend/tests`：348 passed；`PYTHONPATH=platform/wecom-archive-core ./.venv312/bin/pytest -q platform/wecom-archive-core/tests`：6 passed。
- 全量小程序 JavaScript `node --check`、JSON 解析、后端 compileall、受影响 WXML 标签平衡检查和 `git diff --check`：通过。
- 根目录混合 pytest 仍会因 backend 与共享平台都使用顶层 `app` 包而发生收集冲突；未部署生产服务器，未重新编译或上传微信体验版，真实 FCP/TTI、弱网请求瀑布、系统字体和原生分享仍需人工验收。

## 2026-09-03：我的工具服务端生产部署

### 本轮完成

- 按生产部署约定，将当前 `backend/app` 中与商机合作、微信群工具、互帮互助、双积分账本、列表分页、加载状态和 PC 运营后台相关的 22 个文件部署到 `/home/ubuntu/teamBuy`。
- 未新增依赖，采用现有镜像热修复方式；API、backend-worker、archive-worker 均生成带时间戳的 hotfix 镜像并完成容器切换。
- 部署前已备份生产 Compose 和后端源码到 `/home/ubuntu/teambuy-backups/20260903174554/`，并保留三个服务的 rollback 镜像标签。

### 验证

- 本地后端回归：348 passed。
- 生产 API、backend-worker、archive-worker 和 PostgreSQL 均 healthy；端口保持 `8004 -> 8000`。
- 生产 `/health`、`/health/db`、群资源、商机合作和互助任务 GET 接口均返回 200。
- 已在生产容器中核对新增路由；互助积分、互助任务、群资源相关 PostgreSQL 表存在。

### 边界

- 未部署小程序前端包；用户仍需在微信开发者工具清缓存、重新编译并上传体验版，手机端才会加载本轮小程序页面和字号改动。

## 2026-09-03：三类小程序内容增加管理员置顶排序

### 本轮完成

- 互帮互助任务、商业合作名片、微信群资源统一增加 `isPinned/pinnedAt/pinnedBy` 服务端字段；旧数据默认未置顶，无需数据迁移。
- 新增管理员专用 `content-pins` 查询与切换接口，并在 PC 运营后台增加“小程序置顶”管理页，可按类型和关键词筛选、置顶或取消置顶。
- 三类公开列表在服务端排序阶段将置顶内容放在前面，再执行分页；置顶不修改 `updatedAt`，不改变发布、审核、有效期、积分、投诉、退款、处罚或互助结算状态。
- 小程序公开任务、群资源、商机名片及通用公开资料 payload 均移除置顶元数据，因此小程序只感知排序，不显示“置顶”两个字。

### 验证与边界

- 新增 `backend/tests/test_content_pinning.py`：覆盖管理员权限、三类列表第一页排序、重复置顶幂等、公开 payload 隐藏字段和非合作名片边界。
- `PYTHONPATH=.:backend ./.venv312/bin/pytest -q backend/tests`：350 passed。
- 后端 `compileall`、PC 运营后台内嵌 JavaScript 语法检查、`git diff --check`：通过。
- 本轮未重新部署生产服务器，未上传微信体验版；当前后端接口改动需下一次明确部署时才会在线生效，小程序仍需用户在微信开发者工具重新编译、上传体验版。

## 2026-09-03：三类内容置顶功能生产部署

### 部署范围

- 按用户明确确认，部署本轮置顶功能涉及的 6 个后端/PC 文件：领域模型、repository、AppService、运营路由、运营 schema 和 PC 运营后台静态页。
- 未上传测试文件、文档、小程序源码或工作区其他未提交修改；未修改生产 `.env`、密钥、媒体目录、数据库卷、Compose 文件或数据库数据。
- 部署前检查生产磁盘可用约 24G，PostgreSQL healthy，端口为 `8004 -> 8000`；备份目录为 `/home/ubuntu/teambuy-backups/20260903205121/`。
- API、backend-worker、archive-worker 均保留 `rollback-20260903205121` 镜像，并切换到对应 `hotfix-20260903205121` 镜像。

### 线上验证

- 三个 teamBuy 服务和 PostgreSQL 均 healthy；本机 `/health`、`/health/db` 和公网 `/health` 均通过。
- 新增置顶路由、三类模型字段和 `AppService.set_content_pin` 已在新 API 容器加载。
- 无管理员 Token 访问置顶接口返回 403，生产管理员 Token 访问 GET 置顶队列返回 200；PC `/ops` 返回 200。
- 互帮互助、微信群资源、商业合作公开列表均可访问，当前公开响应不含置顶元数据；最近 100 行服务日志未发现 traceback/exception/fatal。

### 边界

- 小程序前端没有部署；用户仍需在微信开发者工具清缓存、重新编译并上传体验版，才会看到三类列表的新排序。

## 2026-09-03：微信群正式目录新增与手机管理员权限

### 本轮完成

- PC 运营后台“业务工作台 → 微信群”新增“新增微信群”入口，写入正式 `GroupResource` 目录，并沿用二维码媒体资产、审核、7 天有效期、投诉、自动暂停、查看积分退款和下架规则；旧“群二维码批量上传”兼容入口保持不变。
- 新增服务端 `group_resource_admin` 角色配置：管理员可在 PC 后台按用户 ID 开通或取消手机管理员权限，手机端只根据服务端权限显示“管理员新增”。
- 手机管理员新增同样进入正式群资源审核链路，不占普通用户每日 1 个发布额度，不发放个人发布奖励；管理员/平台录入资源发生无效处理时，不产生个人处罚欠分或暂停个人发布。
- 公开小程序 payload 不返回管理员来源元数据；PC 审核工作台保留来源标识，便于追溯操作人。

### 验证与边界

- 新增 `backend/tests/test_group_resource_admin.py`：覆盖未授权、PC 配置、手机管理员绕过个人额度、权限撤销即时生效、PC 正式目录写入、审核无个人奖励和公开字段隔离。
- 目标后端测试、全量后端测试、Python compileall、手机端 JS/PC 内嵌 JavaScript 语法检查和 `git diff --check` 均通过；全量后端基线为 353 passed。
- 本轮未部署生产服务器，未上传微信体验版；PC 页面和后端接口需后续明确部署后生效，手机端需用户在微信开发者工具重新编译、上传体验版。

## 2026-09-03：推荐分润与商户转账数据分析

### 本轮完成

- 核对确认系统已有推荐关系、分润奖励和微信商户转账记录，但原 PC 数据分析没有统一查看区域。
- 新增管理员接口 `/api/ops-admin/referral-analytics`，基于正式 `referral_relations`、`referral_rewards` 和 `referral_withdrawals` 生成推荐关系、收益排行、分润明细和转账明细。
- PC“数据分析”下新增“推荐分润”子 Tab，支持今日、近 7 日、总计和按推荐人/被推荐人 ID 或昵称筛选。
- 顶部指标区分推荐关系、产生分润、净分润、提现申请、实际到账和当前可提现余额；明细同时展示“谁推荐谁”和商户转账状态。

### 统计口径

- 推荐关系按绑定时间；产生分润按奖励 `createdAt`；撤销按 `revokedAt`；提现申请按申请 `createdAt`；实际到账只统计 `status=paid` 且 `paidAt` 在区间内的记录。
- 近 7 日按东八区自然日含今天共 7 天；总计兼容历史记录缺失 `paidAt/revokedAt` 的情况。
- 关键词只过滤明细和推荐人排行，不改变顶部时间范围总量，避免筛选后误认为平台总收益发生变化。

### 验证与边界

- 新增 `backend/tests/test_referral_analytics.py`，覆盖管理员鉴权、今日/近 7 日/总计、推荐关系、收益、实际到账、历史兼容和关键词筛选。
- 相关推荐、支付/提现测试：34 passed；全量后端测试：354 passed；compileall、PC 内嵌 JavaScript 语法检查和 `git diff --check` 通过。
- 本轮未部署生产服务器；PC 页面和接口需后续明确部署后生效，生产商户转账数据仍以线上数据库为准。

## 2026-09-03：PC 推荐分润提现审核入口

### 本轮完成

- 在 PC“数据分析 → 推荐分润”中新增“提现审核”区域，默认展示待审核提现，可按待用户确认、转账中、已到账、失败、已撤销和全部记录筛选。
- 接通现有管理员接口：审核并发起微信转账、查询微信状态、人工确认已到账、撤销提现；用户、金额、申请状态、微信状态和商户单号集中展示。
- 发起转账、人工确认到账和撤销均增加浏览器二次确认；人工确认到账额外提示仅限已核实微信到账的异常补记场景，操作后刷新审核列表和推荐分润统计。

### 验证与边界

- PC 内嵌 JavaScript `node --check`、`git diff --check` 通过。
- 本轮未修改提现金额冻结/释放和微信商户转账状态机，未部署生产；上线后需使用真实管理员 Token 在 PC 验证待审核、转账中、查询、撤销和异常人工到账场景。

## 2026-09-03：近期未部署后端与 PC 功能生产发布

### 发布范围

- 按用户明确要求，将当前交接中尚未部署且已核对为线上缺失的 7 个后端/PC 文件发布：群资源正式路由与 schema、群管理员/置顶/推荐分润/提现相关模型与服务、运营路由/schema、PC 运营后台静态页。
- 通过当前镜像热修复注入并切换 API、backend-worker、archive-worker；未覆盖整个脏工作区，未上传小程序源码，未修改生产 `.env`、密钥、媒体目录、数据库卷或生产数据。
- 生产备份目录：`/home/ubuntu/teambuy-backups/20260903221424/`；旧镜像已保留 `rollback-20260903221424` 标签。

### 线上验证

- 三个 teamBuy 服务和 PostgreSQL 均 healthy，Compose 端口保持 `8004 -> 8000`；本机和公网 `/health`、`/health/db` 通过。
- PC `/ops` 已包含提现审核入口；管理员 Token 访问提现列表和推荐分润分析均返回 200，未授权访问返回 403；公开群资源列表返回 200。
- 注入文件的本地/容器 SHA-256 已核对一致，最近 3 分钟后端日志未发现 traceback、fatal 或未处理异常。

### 边界

- 小程序前端仍需用户在微信开发者工具清缓存、重新编译并上传体验版；本次服务器发布不会让小程序前端字号、群管理员新增入口或列表排序自动更新。

## 2026-09-04：用户内容安全与违禁词配置方案

### 本轮完成

- 盘点当前用户内容入口和已有安全能力：`text_safety.py` 目前主要清理非法 Unicode surrogate，项目尚无统一的违禁词规则库、命中记录和审核队列。
- 新增方案文档 `docs/stage2-docs/46-content-safety-forbidden-words-v1.md`，明确统一规范化、规则扫描、拦截/审核/警告决策、发布门禁、公开读取、历史复检、版本一致性、PC 配置和验收边界。
- 方案覆盖资料/合集、名片、商机合作、微信群/活码、互助任务/提交/评论、反馈和企业微信导入公开投影；明确图片、二维码、OCR、外链为后续独立安全策略。

### 当前状态

- 本轮只完成方案设计，没有修改业务代码、数据库、线上规则或生产内容。
- 未运行业务测试，因为没有代码变更；正式实现前必须先补规则引擎、审核记录和全链路测试，再进入报告模式。

## 2026-09-04：用户内容安全与违禁词配置第一版实现

### 本轮完成

- 新增后端 `ContentSafetyService`、规则/判定/审计模型、Json/Postgres repository 支持和明确数据库表；规则按规范化后的短语哈希去重，判定绑定内容指纹、内容 revision 和 rule version。
- 接入资料、合集、名片、商机、供需卡、微信群资源、活码、互助任务/提交/评论、企业微信导入公开投影等高暴露入口；保存、发布和公开读取均由服务端执行最终门禁。
- 处理动作固定为 `block`、`review`、`warn`；普通用户只收到稳定错误码、状态、字段路径和类别摘要，不返回具体命中词。审核批准只对相同内容指纹和规则版本生效。
- PC“运营工作台 → 内容安全”增加规则配置、规则试测、审核队列、放行/拦截和基础统计；审核放行会释放处于复核状态的互助评论和提交。
- 公开列表/详情及历史活码解析入口会重新校验当前有效规则，命中阻断或复核的内容不再公开；公开读取的正常路径不写入审核记录，避免首屏请求产生写放大。

### 验证与边界

- `PYTHONPATH=.:backend ./.venv312/bin/pytest -q backend/tests`：358 passed。
- 内容安全专项测试覆盖 Unicode 隐形字符、字段作用域、命中词隐私、规则 API 鉴权、内容阻断、人工放行跨生命周期 revision 和互助任务审核闭环；专项文件共 4 passed。
- 修改后的 Python 文件 `py_compile`、PC 内嵌 JavaScript 语法检查和 `git diff --check` 通过；应用路由导入通过。
- 未导入任何平台默认违禁词，未部署生产，未上传小程序体验版。上线前必须由运营在 PC 配置经过业务确认的规则，并由用户在微信开发者工具重新编译验收。
- 图片/二维码内容识别、OCR、外链安全、规则批量导入和历史全量异步复检 worker 尚未实现，仍需单独设计供应商、成本、误杀和数据留存策略。

## 2026-09-04：首批违禁词范围收缩确认

- 用户确认此前建议的泛化人工审核词暂不加入，包括贷款、兼职、代理、招商、返现、引流、代运营、代做任务、薅羊毛和解封等。
- 首批后续只保留高确定性风险规则候选；目前仍未向本地或生产导入任何正式违禁词，避免在没有业务样本和用户确认时误伤正常合作内容。

## 2026-09-04：内容安全第一版生产部署

### 发布范围

- 按用户明确要求，将内容安全第一版后端与 PC 管理页面发布到生产；只同步本轮明确的 10 个后端/PC 文件，不同步整个脏工作区，不上传小程序。
- 发布包含规则/判定/审计模型与 repository、`ContentSafetyService`、AppService 门禁、运营 API、活码公开门禁、互助状态回传、数据库 schema 和 PC“内容安全”页面。
- 未导入任何正式违禁词，生产当前规则数量为 0、审核记录数量为 0；泛化人工审核词也没有被配置。

### 生产备份与验证

- 备份目录：`/home/ubuntu/teambuy-backups/20260904055511/`。
- 旧 API、backend-worker、archive-worker 镜像均保留 `rollback-20260904055511` 标签；新镜像使用 `content-safety` 标签并已切换为 Compose 使用的 latest。
- API、backend-worker、archive-worker 和 PostgreSQL 均 healthy；Compose 端口保持 `8004 -> 8000`，本机/公网 `/health` 与 `/health/db` 通过。
- 内容安全管理员接口未带 Token 返回 403，PC `/ops` 已包含“内容安全”，三个运行容器的关键文件 SHA-256 与本地一致，近期 API 日志未发现 traceback/fatal/error。

### 发布边界

- 生产只完成代码和数据库表自动初始化，不代表已经启用内容策略；后续需管理员在 PC 先配置并试测正式规则。
- 小程序源码未上传，用户仍需在微信开发者工具重新编译并上传体验版；生产部署不会自动更新手机端。

## 2026-09-04：资料分享卡底部文案与字号调整

- 根据用户确认，将资料卡底部固定文案从“资料整理助手 · 点击查看完整资料”改为“少来回解释，资料一页讲清”，避免与微信原生标题的查看提示重复。
- 图片型资料卡、无图信息型资料卡和电子名片底部字号统一调整为 30；互助任务卡继续保留“不用加微信，选任务就能互助”系列文案，不被资料卡文案覆盖。
- 分享模板版本从 `share_card_v10` / `share_template_business_collection_v4` 升级为 `share_card_v11` / `share_template_business_collection_v5`，旧分享快照会按版本重新生成。

### 验证与边界

- `business-card-share.js`、`share-snapshot/index.js` Node 语法检查和 `git diff --check` 通过。
- 真机/微信开发者工具仍需人工确认 5:4 图片在有图房源、无图资料、合集和电子名片中的实际字号、截断和换行效果；本轮未上传小程序体验版。

## 2026-09-04：分享卡核心文案字号上调到 32

- 根据用户对比两张分享卡后的反馈，将互助任务卡“不用加微信，选任务就能互助”与资料卡“少来回解释，资料一页讲清”的底部核心文案字号统一调整为 32；资料卡原有 30px 版本不再作为当前模板。
- 分享模板版本升级为 `share_card_v12` / `share_template_business_collection_v6`，避免继续复用 30px 的旧快照。
- 已通过两个相关小程序 JS 文件的 Node 语法检查和 `git diff --check`；仍需用户在微信开发者工具中重新编译后确认实际换行、截断和视觉比例。

## 2026-09-04：互助任务提交、评论与自执行规则收口

- 小程序任务没有可靠的外部完成检测，因此普通提交链路保持服务端确认提交记录；小程序任务在提交成功且未进入内容安全复核时立即结算，执行者直接获得基础积分，并在小程序弹窗中显示“已获得 +N 基础积分”，随后返回任务大厅。
- 普通任务仍保留发布者验收和 1 天自动通过；提交时先预留发布者每次成本，验收通过或自动通过才发放执行者基础积分，退回提交时释放预留积分；执行者奖励不能高于发布者每次成本。
- 用户可以执行自己发布的任务，按普通用户处理，不增加自执行标记、特殊限制或特殊统计；自己的提交同样可以评论、验收和结算。
- 任务详情移除显眼的“分享任务到微信群”按钮，仅保留微信原生右上角分享；普通/小程序任务提交完成记录后才能评论，羊毛任务按解锁后可评论处理。
- 任务列表只公开返回 `published`；详情遇到服务端 404/410 时删除对应本地快照并返回任务大厅，避免用户点击到已删除或已暂停任务的旧卡片。

### 验证与边界

- `PYTHONPATH=.:backend ./.venv312/bin/pytest -q backend/tests`：360 passed。
- 互助相关前端 JS `node --check`、后端 `compileall` 和 `git diff --check` 通过。
- 本轮未部署生产，未上传小程序体验版；用户需在微信开发者工具重新编译后人工确认小程序即时到账、普通任务待验收/退回、自己执行、评论入口和失效任务提示。

## 2026-09-04：互助任务改动部署生产

- 用户明确确认部署后，按腾讯云现有镜像热修复约定，将互助任务后端相关的 `domain.py`、`scrm.py`、`routes_scrm.py` 和 `app_service.py` 部署到生产 API、backend-worker、archive-worker 镜像。
- 生产备份目录：`/home/ubuntu/teambuy-backups/20260904130242/`；旧 API、两个 worker 的回滚标签为 `rollback-20260904130242`。生产 `.env`、密钥、媒体目录和数据库卷未覆盖。
- 新镜像使用 `mutual-help-fixed-20260904130242` 标签并切换为 Compose 使用的 latest；API 启动命令已核对为 uvicorn，两个 worker 启动命令已核对为 `python -m app.worker`。

### 生产验证

- `teambuy-backend-1`、`teambuy-backend-worker-1`、`teambuy-archive-worker-1`、`teambuy-postgres-1` 均 healthy。
- 端口仍为 `8004 -> 8000`；本机 `/health`、`/health/db` 和公网 `https://teambuy.lifelove.top/health` 全部通过。
- 运行容器四个目标文件 SHA-256 与本地一致，互助退回提交路由存在，API 启动日志无 Traceback/ERROR。
- 本轮没有上传小程序体验版；小程序前端改动仍需用户在微信开发者工具重新编译上传。

## 2026-09-04：互助任务参与频次策略落地

- 发布任务页新增“每位用户可参与”双按钮：默认“仅限一次”，可选“每天一次”；羊毛任务不显示该设置，因为羊毛是查看/解锁资源而不是提交完成任务。
- 任务模型和发布接口新增 `repeatPolicy`，历史任务缺省兼容为 `once`。服务端使用 Asia/Shanghai 自然日记录 `participationDay`，同一用户同一任务同一天幂等，跨天可重新参与；任务总剩余名额仍按每次成功结算扣减。
- 任务大厅和详情页会显示参与频次；每日任务当天完成后继续保留在大厅，并提示“今日已完成，明日可参与”，不会误显示成永久消失。
- 发布页辅助说明、帮助文字和占位文字颜色适度加深，双按钮使用 flex 居中、不可换行，保持真机可读性。

### 验证与边界

- `PYTHONPATH=. ./.venv312/bin/pytest -q backend/tests`：362 passed。
- 互助发布、任务大厅、任务详情相关 JS `node --check`，后端 `compileall` 和 `git diff --check` 通过。
- 从仓库根目录直接运行 `pytest -q` 仍会触发项目既有的跨目录测试收集错误；按项目实际入口带 `PYTHONPATH` 执行的 `backend/tests` 全量回归已通过。
- 本轮未部署生产，未上传小程序体验版；用户需在微信开发者工具重新编译，重点验收发布页两按钮、每日任务跨设备状态和北京时间 00:00 后再次参与。

## 2026-09-04：商机合作独立发布入口与合作机会表单

### 本轮完成

- 商机合作页新增“发布合作信息”入口和底部选择层，继续保留原有三个 Tab；用户可选择进入名片编辑，或进入独立“发布合作机会”页面。
- 新表单采用最小字段：我能提供/我正在找、标题、具体说明、联系方式和展示有效期。标题占位符在空值时居中，输入内容后左对齐。
- 联系方式默认使用合作名片的电话/微信；名片不存在或没有可用联系方式时，显示手动填写电话/微信。服务端不向公开列表返回原始联系方式，详情解锁统一扣 1 个基础积分并复用 24 小时幂等记录。
- 用户发布的供需卡接入商机合作的“合作资源/合作需求”列表，点击进入供需详情；列表继续采用两路游标分页，并修正某一路耗尽后重复请求第一页的问题。
- 增加独立合作机会的有效期字段和过期过滤；编辑切换“长期有效”时会真正清除已有到期时间。保存/提交继续复用现有内容安全和人工审核门禁。
- 供需详情允许匿名先浏览，登录后才能申请或解锁联系方式；解锁后支持复制微信、拨打电话。

### 验证与边界

- `./.venv312/bin/pytest -q backend/tests/test_business_opportunity_posts.py backend/tests/test_app.py -k 'business_opportunity or supply_demand'`：4 passed。
- 新增后端覆盖：手填联系方式脱敏、审核前不公开、详情解锁一次扣分、24 小时重复解锁不重复扣分、名片联系方式和无联系方式时的服务端校验、长期有效清除旧期限。
- 商机合作、新发布页、商机名片详情、供需详情和 API JS 均通过 `node --check`；后端 `compileall` 与 `git diff --check` 通过；相关页面 JSON 解析通过。
- 本轮未部署生产服务器，未上传微信体验版。用户需在微信开发者工具重新编译，重点人工验收底部选择层、标题居中占位、无名片手填、名片联系方式、审核后列表展示、详情解锁和过期状态。

## 2026-09-04：修正商机合作空态误显示“制作名片”

- 根因是合作需求/合作资源列表的空态按钮写死为“制作我的名片”，同时页面只在“我的名片”Tab 读取名片摘要，导致已有名片的用户也看到错误 CTA。
- 商机合作页现在在所有 Tab 读取轻量名片摘要；名片确认完成前显示“正在确认你的合作名片…”，已有名片时空态按钮改为“发布合作信息”，没有名片时才显示“制作我的名片”。
- 增加名片异步请求的用户身份竞态保护，避免切换账号后旧请求覆盖当前用户的空态判断。

### 验证

- 商机合作页 Node 语法检查、后端 compileall、`git diff --check` 和 `app.json` JSON 校验通过。
 - 未部署生产，未上传微信体验版；需在微信开发者工具重新编译后，分别验收已有名片、无名片、列表为空和点击空态按钮的结果。

## 2026-09-04：PC 商机线索录入表单简化

- 用户反馈原“录入商机线索”页面把线索 ID、状态、可信状态、联系方式状态、来源和核心内容混在一起，无法直观看出该怎么录入。
- 录入页改为单张“核心录入”卡片：标题明确标记为必填，摘要和完整内容使用带示例的选填/建议填写占位符，联系方式单独突出并标明选填。
- 线索 ID 改为隐藏字段，新建时由服务端自动生成；脱敏展示由服务端自动生成，不再要求管理员填写。
- 城市、区域、行业、合作方向、标签、来源记录和发布设置改为可展开的补充区域；新增“仅保存草稿”和“保存并发布”两个明确操作。
- 保存时若来源原文为空，自动使用完整内容作为后台留档，减少同一段微信群消息重复粘贴；保存后表单和折叠区域恢复到新建状态。

### 验证与边界

- 内嵌 JavaScript node --check 通过；HTML ID 唯一性和商机表单关键控件存在性检查通过；git diff --check 通过。
- 本轮只修改 PC 运营后台静态页面，未部署生产；上线后仍需在 PC 页面人工验证新建、草稿、发布、点击列表行编辑和下架流程。

## 2026-09-04：我的工具 PC 运营工作台 V1 开发完成（本地未部署）

### 本轮完成

- PC 左侧导航按“我的工具、权益与计费、群运营、运营配置、用户与反馈”重新归组；新增“我的工具 · 数据总览”，支持今日、近 7 日、累计三种范围。
- 总览以服务端已落库数据和共享积分流水为唯一口径，展示基础积分增加、充值积分增加、积分消费，以及互帮互助、微信群工具、商机合作的任务/资源/线索、发布、提交/查看、投诉和联系方式解锁数据。
- 新增三类工具统一明细接口和页面筛选：关键词、状态、仅看测试数据、分页；支持当前页全选、标记测试、批量下架和测试数据删除。
- 批量清理默认是可恢复下架；永久删除必须同时勾选“仅看测试数据”，且记录必须显式标记为测试数据、没有积分/提交/投诉/回应包/联系方式/匹配/解锁等业务关联。微信群下架保留为 `reviewStatus=removed`，可走既有恢复流程。
- 新增“权益与计费”统一页，把充值积分、商户转账/提现、会员与客户信息链的功能/显示配置放在一起；原互助积分配置面板隐藏，避免同一开关有两个入口。
- 新增统一手机管理员配置表，可按用户 ID 配置互帮互助、微信群工具、商机合作角色；微信群角色继续接入已有手机管理员新增链路，其他两项因普通手机发布本身已开放，当前角色作为统一权限台账预留。
- 新增 PC“管理员发布互助任务”最小表单；不填写普通用户 ID 时由平台管理员作为内部归属，仍走原有完成标准、积分和内容安全规则。商机线索录入、微信群新增入口沿用已存在的后台表单。

### 验证与边界

- `PYTHONPATH=.:backend ./.venv312/bin/pytest -q backend/tests`：369 passed。
- 新增工作台专项测试 4 passed；Python `compileall`、PC 内嵌 JavaScript `node --check`、HTML 字面量 ID 检查和 `git diff --check` 通过。
- 直接从仓库根目录运行 `pytest -q backend/tests` 仍会触发项目既有的 `backend.tests` 导入路径问题；按项目约定加 `PYTHONPATH=.:backend` 执行。
- 本轮只修改本地代码，未部署生产，未上传微信体验版。PC 需要真实管理员 Token 人工验证；小程序仍需用户在微信开发者工具重新编译上传。

## 2026-09-04：商机合作独立发布逻辑部署生产

- 用户明确确认部署生产。本次只发布商机合作独立发布及供需卡接入所需的四个后端文件：`backend/app/api/routes_opportunities.py`、`backend/app/models/domain.py`、`backend/app/schemas/opportunities.py`、`backend/app/services/app_service.py`；没有把未提交工作区整体同步到服务器。
- 采用现有“当前镜像范围热修复”约定，API、backend-worker、archive-worker 均已切换；生产 `backend/.env`、secrets、媒体目录、数据库卷和 `docker-compose.yml` 均未被覆盖。
- 生产备份目录：`/home/ubuntu/teambuy-backups/20260904211735/`；旧 API、backend-worker、archive-worker 镜像分别保留 `teambuy-rollback-api:20260904211735`、`teambuy-rollback-backend-worker:20260904211735`、`teambuy-rollback-archive-worker:20260904211735`；当前镜像另保留 `teambuy-deploy-api:20260904211735`、`teambuy-deploy-backend-worker:20260904211735`、`teambuy-deploy-archive-worker:20260904211735`。

### 生产验证

- `teambuy-backend-1`、`teambuy-backend-worker-1`、`teambuy-archive-worker-1`、`teambuy-postgres-1` 均 healthy，端口保持 `8004 -> 8000`。
- 本机 `/health`、`/health/db` 与公网 `https://teambuy.lifelove.top/health` 通过；OpenAPI 已确认商机合作列表/详情和供需联系方式解锁路由存在。
- 三个运行容器内四个目标文件的 SHA-256 均与本地一致，API 启动日志无 Traceback/ERROR。
- 小程序前端没有通过服务器更新；最新的商机合作空态按钮修正和合作机会页面，仍需用户在微信开发者工具重新编译、上传体验版后人工验收。

### 2026-09-04：PC 我的工具工作台 V1 部署生产

- 已按生产部署约定发布本轮明确的后端/PC 文件：`backend/app/api/routes_ops_admin.py`、`backend/app/models/domain.py`、`backend/app/schemas/ops_admin.py`、`backend/app/services/app_service.py`、`backend/app/static/ops-admin/index.html`。
- 生产备份目录：`/home/ubuntu/teambuy-backups/20260904231407/`；旧 API、backend-worker、archive-worker 镜像分别保留为 `teambuy-rollback-api:20260904231407`、`teambuy-rollback-worker:20260904231407`、`teambuy-rollback-archive:20260904231407`。
- 当前 API、backend-worker、archive-worker 镜像均已带本次时间戳发布标签，并由 Compose 使用；未修改生产 `backend/.env`、secrets、媒体目录和数据库卷。

### 生产验证

- `teambuy-backend-1`、`teambuy-backend-worker-1`、`teambuy-archive-worker-1`、`teambuy-postgres-1` 均 healthy，端口保持 `8004 -> 8000`。
- 本机与公网 `/health`、`/health/db` 均通过；新增工作台、手机管理员、平台互助任务路由均返回受保护的 `403`（说明路由已注册且未授权请求未执行管理操作）。
- 新页面标记、服务端关键符号及五个目标文件的 SHA-256 均已在生产容器内核对，与本地发布文件一致；最近 100 行三个应用容器日志无 `Traceback`、`FATAL` 或 `ERROR`。
- 小程序前端不会通过服务器自动更新；如需小程序侧生效，仍需用户在微信开发者工具中重新编译并上传体验版。

## 2026-09-05：充值积分双账本、任务预算与提现审核本地开发

### 本轮完成

- 后端补齐充值积分任务的发布预留、验收发放、拒绝释放和删除释放；执行者获得的充值积分进入可提现账本。
- 普通任务和小程序任务新增基础积分/充值积分奖励选择；羊毛福利保留无固定完成奖励的既有规则。
- 新增平台充值积分任务预算及 PC 调整入口，平台管理员发布充值积分任务从独立预算扣减。
- 新增小程序充值积分页与提现页；提现最低 100 充值积分、默认 20% 手续费、无 7 天等待，申请后进入 PC 审核队列。
- PC“数据分析 → 推荐分润”补充独立的“充值积分提现审核”区域，支持审核转账、查单、人工确认、撤销/失败处理。
- 修正旧平台奖励迁移兼容边界，避免把新的任务奖励、平台预算和提现流水迁移到基础积分。

### 本地验证

- 充值积分双账本、任务预算、提现审核及原互助回归：`20 passed`。
- `backend/app` Python compileall 通过；PC 内联脚本和相关小程序 JS `node --check` 通过；`git diff --check` 通过。

### 未执行

- 未部署生产服务器，未修改生产环境变量、密钥、媒体或数据库运行态数据。
- 小程序未上传体验版；需要用户在微信开发者工具重新编译后人工验收充值、提现、任务发布/验收和 PC 审核联动。

## 2026-09-06：我的账户入口与提现规则补齐

- 在“我的”页面的“我的工具”四格入口中新增“我的账户”，链接到互助分包账户页；不增加新的业务工具入口。
- 账户页复用服务端双账本状态，展示总积分、基础积分、充值积分、充值/提现按钮和最近流水。
- 充值/提现按钮分别受 PC“权益与计费”中的显示开关和功能开关控制；提现独立页同步展示完整审核规则。
- 后端提现规则新增提现范围、兑换比例、最低金额、手续费、每日次数、审核到账和失败退回等结构化文案。
- 调整小程序显示逻辑：提现显示开关关闭时同步隐藏提现按钮与提现说明；提现功能关闭但显示开关开启时，保留规则并将按钮置为不可操作状态。
- 本轮相关测试 `20 passed`，小程序 JS、JSON、Python compileall 和 `git diff --check` 均通过；未部署生产，未上传小程序体验版。

## 2026-09-06：充值积分与账户页后端/PC 生产部署

### 发布范围

- 用户已明确确认部署生产。本次只发布以下 9 个后端/PC 文件：`backend/app/api/routes_ops_admin.py`、`backend/app/api/routes_scrm.py`、`backend/app/core/schema.sql`、`backend/app/models/domain.py`、`backend/app/schemas/ops_admin.py`、`backend/app/schemas/scrm.py`、`backend/app/services/app_service.py`、`backend/app/services/repository.py`、`backend/app/static/ops-admin/index.html`。
- 未同步整个未提交工作区，未上传小程序源码；生产 `backend/.env`、secrets、媒体目录、PostgreSQL 数据库和数据卷未覆盖。
- 生产备份目录：`/home/ubuntu/teambuy-backups/20260906002920/`。旧 API、backend-worker、archive-worker 回滚标签分别为 `teambuy-rollback-api:20260906002920`、`teambuy-rollback-worker:20260906002920`、`teambuy-rollback-archive:20260906002920`。
- 当前发布镜像分别为 `teambuy-deploy-api:20260906002920`、`teambuy-deploy-worker:20260906002920`、`teambuy-deploy-archive:20260906002920`，并已切换为 Compose 使用的 `latest` 标签。

### 生产验证

- API、backend-worker、archive-worker、PostgreSQL 均 healthy；生产端口保持 `8004 -> 8000`。
- 本机与公网 `/health`、`/health/db` 均通过；PC `/ops` 已包含“我的工具”“权益与计费”和“提现审核”。
- 新增提现接口未授权返回 `401/403`，平台预算接口方法/权限检查符合预期；生产数据库已存在 `mutual_point_withdrawals`、双账本表及提现所需字段。
- 9 个发布文件在三个运行容器内的 SHA-256 均与本地发布文件一致；最近 4 分钟 API、两个 worker 日志未发现 `Traceback`、`FATAL` 或 `ERROR`。

### 发布边界

- 小程序账户入口、充值页、提现说明和显示开关未通过服务器更新；负责人仍需在微信开发者工具清缓存、重新编译并上传体验版。
- 本轮没有创建生产充值订单或真实提现，没有调用微信商户转账；真实支付/提现必须在具备商户配置的环境中人工验收。

## 2026-09-06：充值积分隐藏状态下的小程序展示收口

- 账户页关闭充值积分显示时改为展示“可用积分”，隐藏双账本说明、积分拆分、充值/提现入口、提现说明及流水中的充值积分标签；服务端余额仍按真实双账本计算。
- “我的工具”页同步隐藏积分拆分和充值/提现入口；充值页、提现页增加历史路径保护，仅显示入口暂未开放提示，不能提交相关操作。
- 本轮仅修改小程序前端和项目文档，未部署生产、未修改服务端数据，仍需用户在微信开发者工具清缓存并重新编译验收。

## 2026-09-06：互帮互助任务大厅、发布、管理三页 UI 重排

- 任务大厅移除重复宣传卡和列表中的入口/发布者信息，保留用户决策所需字段；排序与布局控件重新分组，布局缓存版本更新为 v3 并默认单列。
- 任务管理移除列表大分享图，增加页面级“发布任务”入口和卡片级“去验收/查看管理”主操作，分享保留为次级按钮，删除降为文字操作。
- 发布页压缩类型说明、保留主题切换、普通任务和羊毛福利的参考入口改为选填折叠；充值积分隐藏时仅提供基础积分奖励。
- 三页同步使用服务端 `rechargeVisible` 控制充值积分展示，并对旧页面路径保留前端操作保护。
- 相关小程序 JS `node --check` 和 `git diff --check` 通过；未部署生产，未上传微信体验版。

## 2026-09-06：微信群“我的发布”入口与发布额度本地开发

- “我的发布”标题区新增“＋发布群”入口，空列表按钮改为“发布第一个群”；找群页的发布引导收缩为轻量入口，避免发布动作与列表混排。
- 新增服务端 `GET /api/group-resources/publish-quota`，按当天已创建记录返回 `dailyLimit`、`usedToday` 和 `remainingToday`；删除记录仍计入当天额度。
- 发布页新增今日剩余额度卡；额度读取中或失败时不伪造可用次数，创建接口仍保留服务端最终限制。
- 群资源卡片删除操作改为与更新二维码同一操作区的等高按钮，文案改为“删除群资源”。

### 本轮验证

- `backend/tests/test_group_tools.py`：4 passed。
- 相关小程序 JS、API JS `node --check`，后端 `compileall`，WXML 结构检查和 `git diff --check` 通过。
- 未部署生产，未上传微信体验版；小程序需用户在微信开发者工具清缓存并重新编译。

## 2026-09-06：微信登录永久 loading 防护

- 登录流程新增 20 秒应用层总超时，避免微信 SDK 或请求回调异常时按钮永久处于“登录中”。
- 登录成功后先保存会话并跳转首页，昵称/头像补充改为后台处理，不再阻塞首次登录。
- 登录后的页面跳转增加失败回退；四类小程序上传接口统一增加 30 秒超时。
- 本地登录成功与超时场景模拟通过；112 个小程序 JS、84 个 JSON、登录测试和后端 372 个测试通过；未部署生产，未上传小程序体验版。

## 2026-09-06：OpenHarmony 请求 complete 回调兼容

- 发现生产登录请求已经返回 200，但 OpenHarmony 微信运行时可能未触发 `wx.request` 的 `success`；请求封装新增 `complete` 响应兜底。
- `complete` 带 HTTP 状态码时按响应统一处理；无响应时仍由失败回调或登录总超时收口。
- complete-only 成功/失败模拟通过；未部署服务器，需重新编译上传小程序体验版。

## 2026-09-06：OpenHarmony 登录再次复现后的根因边界

- 用户 15:04 左右再次测试；生产后端立即记录 `POST /api/auth/wechat-login` `200 OK`，Nginx 记录同一设备请求返回 `200 / 1120 bytes`。
- 登录后没有后续 API 请求，说明小程序尚未进入 `saveLogin`；服务器、微信 `jscode2session` 和 PC 部署不是当前已证实的阻断点。
- 当前唯一未闭环证据是 OpenHarmony 微信真机的 Network/Console：需确认实际运行包是否包含最新请求封装，以及 200 响应后 `success/complete` 是否交给 JS。暂不继续增加超时或服务器层防护。

## 2026-09-06：开发版鸿蒙异常与体验版可用的边界

- 用户反馈同一鸿蒙设备在开发版/真机调试中登录异常，但上传体验版后可以正常登录。
- 当前不新增鸿蒙专用回调式登录协议、额外响应头或服务器部署；现有登录超时只保留为防止永久 loading 的通用兜底。
- 后续验收以体验版为准；只有体验版仍能稳定复现时，才继续做客户端兼容补丁。

## 2026-09-06：微信群发布额度后端部署生产

- 本次只部署此前未上线的两处后端改动：`backend/app/services/app_service.py` 和 `backend/app/api/routes_group_resources.py`，没有同步整个未提交工作区，也没有发布小程序源码。
- 生产备份目录：`/home/ubuntu/teambuy-backups/20260906153836/`；包含 Compose、生产环境文件备份和两份旧源码备份。
- 三个应用服务通过带时间戳的补丁镜像切换并保留回滚标签：`teambuy-deploy-api:20260906153836`、`teambuy-deploy-worker:20260906153836`、`teambuy-deploy-archive:20260906153836`；对应回滚标签分别为 `teambuy-rollback-api:20260906153836`、`teambuy-rollback-worker:20260906153836`、`teambuy-rollback-archive:20260906153836`。
- 本地全量后端测试 `372 passed`；生产 API、backend-worker、archive-worker 和 PostgreSQL 均 healthy，`8004 -> 8000` 映射、本机/公网 `/health` 与 `/health/db` 均通过。
- `GET /api/group-resources/publish-quota` 已在生产生效，未登录请求返回 `401`；容器内两份部署文件哈希与本地一致，最近日志未发现 `Traceback`、`FATAL` 或 `ERROR`。
- 小程序仍需用户在微信开发者工具清缓存、重新编译并上传体验版；这次服务器部署不会更新小程序前端。

## 2026-09-06：微信群工具体验版问题核查

- 核对生产公开数据后确认，“全国综合群”是 2026-09-02 按旧规则保存的 `rewardAmount=20`，状态为已到账；当前代码和新建记录规则已经是发布奖励 `10`，历史已到账流水不能直接改金额。
- 发现小程序“我的发布”无条件把 `rewardAmount` 显示为“审核中 +N”，导致已到账记录也显示审核中；需要按 `rewardState` 区分待审核、已到账和奖励上限。
- 发现发布表单仍把湖南/长沙写死为默认城市，行业选项仍从“房产”开始，当前没有“互助”行业和自定义标签的数据模型、接口或页面入口。
- 发现删除群资源与更新二维码使用不同的 flex 宽度规则，操作按钮无法保证等宽；这是明确的 UI 排版问题。
- 本轮只核查未改代码；群工具专项测试 `13 passed`，相关 JS 检查和 `git diff --check` 通过。

## 2026-09-06：微信群工具体验版问题收口（本地未部署）

- 小程序奖励文案改为按 `rewardState` 展示：待发放显示“审核中 +N”，已到账显示“已到账 +N”，达到上限不显示虚假的待审核奖励；历史 `+20` 记录保留原始账本金额。
- 群资源删除改为只取消尚未发放的奖励并释放累计奖励占用，已到账奖励不回写、不改流水；删除记录仍计入当天新发布额度，防止发布后删除再重发绕过限制。
- 发布表单默认改为“全国”和“互助”；小程序和 PC 管理员录入均支持最多 8 个自定义标签，标签写入领域模型、接口、内容安全检查、公开搜索和管理搜索。
- PC 管理员新增通过内容安全检查后直接进入找群，不再显示为待人工审核；历史管理员记录在无人工审核日志时也按已发布展示。管理员更新二维码不会被普通用户的奖励审核逻辑误置为审核中。
- 删除与更新二维码按钮统一为同一操作区、同高、等宽并保持文字居中；发布成功页和公开/我的发布卡片会展示标签。

### 本轮验证

- 全量后端测试：`373 passed`；群资源专项回归：`14 passed`。
- 后端 `compileall`、群资源小程序 JS/API JS `node --check`、PC 内联脚本解析、WXML 事件绑定检查、`git diff --check`：通过。
- 本轮未部署生产、未上传微信体验版；需在微信开发者工具清缓存、重新编译并上传体验版后人工验收。

## 2026-09-06：微信群工具问题收口部署生产

- 生产部署范围收口为 6 个明确文件：`app_service.py`、`domain.py`、`repository.py`、`schemas/group_resources.py`、`routes_group_resources.py` 和 PC 管理页 `static/ops-admin/index.html`；未同步整个未提交工作区，也未覆盖生产 `.env`、`secrets`、媒体目录或数据库。
- 生产 Compose 实际为构建型配置、没有源码绑定；先前同步宿主机源码不会让运行中的 Python 进程加载新代码。本次改用当前生产镜像做文件层热修，生成 `teambuy-release-api/worker/archive:20260906171337` 并强制重建三个应用服务。
- 生产备份：`/home/ubuntu/teambuy-backups/20260906171337-group-resource-closeout/`；回滚标签：`teambuy-rollback-api/worker/archive:20260906171337`。
- 部署后 API、backend-worker、archive-worker、PostgreSQL 均 healthy；`8004 -> 8000`、本机/公网 `/health`、`/health/db`、公开群资源列表和 OpenAPI 群资源路由均通过；容器内 6 个文件哈希与本地一致。
- 小程序前端仍未通过服务器部署；用户需在微信开发者工具清缓存、重新编译并上传体验版。

## 2026-09-06：羊毛任务卡片动作文案修正

- 任务大厅卡片按任务类型显示动作：羊毛使用“去查看”，普通任务和小程序任务继续使用“去完成”，避免把福利查看误解为提交任务。
- 本轮仅修改小程序前端，未部署生产；需随下一次体验版上传验收。

## 2026-09-06：互帮互助演示任务清理（本地未部署）

- 删除小程序内置的 4 条 `DEMO_TASKS`，服务器没有真实任务时不再自动展示招聘、校对、资料体验和羊毛示例卡片。
- `getTasks()` 会清理已有设备缓存中的 `demo-*` 任务；`mergeServerTasks()` 不再把本地演示任务重新拼回服务器结果。
- 生产服务器核对确认互帮互助公开任务接口当前返回 0 条；小程序需重新编译并上传体验版后，任务大厅才会在用户设备上执行新的清理逻辑。

### 验证

- 目标页面和共享模块 `node --check` 通过。
- 模拟已有演示缓存、空服务器响应的 cleanup smoke 通过。
- `git diff --check` 通过；本轮未部署生产、未上传微信体验版。

## 2026-09-06：测试数据清理策略收口（本地未部署）

- PC “清理测试数据”按关联流水自动分流：无提交、积分、查看、投诉或其他业务关联的记录永久删除；已有业务关联的记录安全下架，保留原记录和流水。
- 批量接口新增 `archived` 和逐条 `outcomes` 返回，PC 明确提示永久删除、安全下架、批量下架和跳过的数量及原因。
- 群资源、互帮互助任务和商机合作均使用服务端状态作为公开/我的发布过滤依据；带流水的测试群使用 `status=deleted/reviewStatus=removed` 隐藏，积分账本和投诉审计不改写。
- 小程序群资源首次读取和重试失败时清空旧列表；互帮互助“任务管理”和“我的工具”同步失败时不再静默展示旧本地发布数据，而是显示重试空态。任务大厅仍保留带明确同步失败提示的本地快照首屏展示。

### 验证

- `backend/tests/test_ops_tools_workbench.py` 与 `backend/tests/test_group_tools.py`：`10 passed`。
- Python 编译、涉及小程序 JS `node --check`、`git diff --check`：通过。
- 本轮未部署生产、未上传微信体验版；生产清理需用户后续明确确认后再执行。

## 2026-09-06：互帮互助任务发布 422 修复（本地未部署）

- 定位到普通/小程序任务发布失败的真实原因：前端将非羊毛任务的 `woolPolicy` 传为 `null`，而后端 schema 要求该字段为对象，因此接口返回 `422 Unprocessable Entity`。
- 小程序请求层新增 FastAPI 校验详情格式化；不再把错误对象直接显示为 `[object Object]`，会显示字段位置和具体原因。
- 非羊毛任务统一传 `woolPolicy: {}`；羊毛任务继续传实际解锁费用和打赏配置。

### 验证

- 互帮互助积分相关后端测试：`12 passed`。
- 发布页和请求层 `node --check`、`git diff --check`：通过。
- 本轮未部署生产、未上传微信体验版；需重新编译上传后验证真机提交。

## 2026-09-06：我的工具页面返回路径补全（本地未部署）

- 互帮互助首页、我的工具、发布任务、任务管理、任务详情、账户、充值、提现和网页中转页统一补充 `fallbackUrl`；有页面栈时仍优先使用系统返回。
- 商机合作首页、详情和发布页补充个人中心/商机首页兜底返回；微信群资源库已有的个人中心兜底和发布流程内部返回保持不变。
- 网页中转页增加自定义导航栏，避免从任务详情打开外部网页后没有左上角返回入口。

### 验证

- 相关页面 JavaScript `node --check`、JSON 解析和 `git diff --check`：通过。
- 本轮未部署生产、未上传微信体验版；需重新编译上传后在真机验证页面栈返回和无页面栈兜底返回。

## 2026-09-07：商机合作行业字典扩充（本地未部署）

- 商机合作的一级行业和细分行业统一收口到 `miniprogram/utils/business-industry.js`，编辑名片页与商机合作筛选页共用同一份字典。
- 保留已有行业值，并补充制造业常用的服装纺织、鞋帽箱包、家电、汽车零部件、五金工具、食品饮料、建材家居、新能源等细分方向。
- 同步补齐外贸、电商、企业服务、物流供应链、本地生活、房产、设计营销、教育培训、健康美业、农业食品、互联网科技、汽车出行、金融保险、文化传媒和能源环保等行业，行业标签仍用于表达更长尾的合作关键词。

### 验证

- 行业字典、名片编辑页和商机合作页 `node --check` 通过。
- 已验证“制造业 → 服装纺织 / 家电”等细分选项能由统一字典生成。
- 本轮未部署生产、未上传微信体验版；需在微信开发者工具重新编译后验收选择和筛选。

## 2026-09-07：PC 运营后台清理与非阻塞提示收口（本地未部署）

- 小程序置顶管理过滤已删除互帮互助任务、已删除群资源和已下架群资源，避免失效内容继续出现在置顶队列。
- 三类工具明细默认隐藏 `deleted`、`archived`、`removed` 记录；服务端仍保留有流水记录供审计，显式状态筛选仍可查看支持的归档状态。
- PC 后台把原生 `alert()` 提示统一替换为右上角页面内通知，成功和失败均不打断页面操作。

### 验证

- `./.venv312/bin/python -m pytest -q backend/tests/test_content_pinning.py backend/tests/test_ops_tools_workbench.py`：`7 passed`。
- PC 后台内嵌 JavaScript 语法检查、`python3 -m py_compile backend/app/services/app_service.py`、`git diff --check`：通过。
- `backend/app/static/ops-admin/index.html` 已确认无 `alert()` 调用。
- 本轮未部署生产。

## 2026-09-07：PC 运营后台清理与非阻塞提示已部署生产

- 已将后端源码和 `docker-compose.yml` 同步到 `81.70.84.35:/home/ubuntu/teamBuy`，未覆盖生产 `backend/.env`、`backend/secrets/`、媒体卷和数据库卷。
- 已在 `/home/ubuntu/teambuy-backups/20260907-035647/` 保存 Compose 与后端源码备份，并为部署前运行中的三个旧镜像建立回滚标签。
- 已重新构建并启动 `teambuy-backend-1`、`teambuy-backend-worker-1`、`teambuy-archive-worker-1`；`teambuy-postgres-1` 未重启。

### 生产验证

- 三个新容器和数据库容器均为 `healthy`，后端端口保持 `8004`。
- `http://127.0.0.1:8004/health` 与 `https://teambuy.lifelove.top/health` 均返回 HTTP 200，数据库连接正常。
- `https://teambuy.lifelove.top/ops` 已返回最新 PC 页面，包含页面内通知实现且无 `alert()`。
- 小程序代码未通过服务器发布；如需生效，仍需在微信开发者工具重新编译并上传体验版。

## 2026-09-07：互帮互助执行者详情页与跨设备任务可用性修复（本地未部署）

- 修复执行者打开发布者任务时的关键误判：执行者设备不再读取本地缓存的发布者积分来决定任务是否结束；任务是否可用以服务端返回的 `taskClosed/status/remaining` 为准。
- 后端互帮互助任务 payload 增加 `taskClosed`，仅由任务状态和剩余次数决定；发布者余额不再成为执行者侧的关闭条件。
- 执行者详情页按“先完成核心动作，再看完整标准”重排：奖励与四项事实信息、三步流程、首要入口、完整任务、完成标准、反馈和提交动作分层展示。
- 已通过 `node --check`、`git diff --check`、Python 编译检查；互帮互助积分相关测试 `12 passed`，应用测试 `186 passed`。
- 本轮未部署生产、未上传微信体验版；需要先部署后端变更，再由微信开发者工具重新编译上传小程序体验版进行真机验收。

## 2026-09-07：三类执行者任务统一详情页（本地未部署）

- 以第二版效果图为统一骨架，完成小程序任务、普通任务、羊毛任务三种详情变体。
- 小程序任务保留“打开小程序→体验→返回提交”；普通任务改为“阅读说明→按标准完成→提交材料”；羊毛任务改为“解锁/查看→使用福利”，不再显示误导性的“提交完成”。
- 羊毛任务增加解锁主操作、福利入口、使用结果摘要和羊毛专属说明；普通任务在无入口时增加“阅读说明并开始”的行动引导。
- 已通过 WXML 标签配对、JavaScript 语法、`git diff --check`，并回归后端测试 `198 passed`。
- 本轮未部署生产、未上传微信体验版。

## 2026-09-07：商机合作与微信群工具打开阻塞收口（本地未部署）

- 商机合作入口不再先请求名片摘要再跳转；页面立即打开，名片状态改为后台加载，只有发布或进入“我的名片”时才引导创建。
- 商机合作列表拆分为主商机列表和供需补充列表；供需接口失败时仍展示主列表，并给出非阻塞重试提示。
- 生产身份白名单补充供需卡片的公开 GET 列表和详情，避免未携带登录态时返回 401 进而影响合作大厅。
- 群资源库先等待公开群资源接口并完成首屏渲染，账户余额、我的发布、管理员状态和发布额度改为后台补齐；页面增加 30 秒状态缓存和进行中请求合并。
- 免费活码入口不再先加载群资源库的其它接口，并增加独立请求合并和短缓存。

### 验证

- 相关小程序 JS 全量 `node --check`：通过。
- `git diff --check`：通过。
- 商机、供需公开接口和群工具专项测试：`7 passed`。
- 后端测试（排除项目内已有导入路径错误的两个测试文件）：`367 passed`。
- 全仓库 pytest 仍有既有收集问题：两个测试文件引用 `backend.tests` 时找不到包，`platform/wecom-archive-core/tests` 依赖独立运行路径；不是本轮业务断言失败。
- 本轮未部署生产、未上传微信体验版；小程序需重新编译上传后真机验收。

## 2026-09-07：商机合作与微信群工具打开链路已部署生产

- 已将本轮后端 `app`、Dockerfile、依赖清单上传到 `81.70.84.35:/home/ubuntu/teamBuy`，未覆盖生产 `backend/.env`、`backend/secrets/`、数据库和媒体卷。
- 部署前备份位于 `/home/ubuntu/teamBuy/.deploy-backups/20260907-1646-tool-entry/`，三个旧应用镜像已建立回滚标签。
- 服务器完整 Docker 构建因 `apt-get update` 网络长时间无进展而停止；随后基于已验证的生产镜像制作源码热修复镜像，并确认 `psycopg_pool` 与 `app.main` 可导入后切换，避免使用不完整的测试镜像。

### 生产验证

- `teambuy-backend-1`、`teambuy-backend-worker-1`、`teambuy-archive-worker-1`、`teambuy-postgres-1` 均为 healthy。
- 后端端口保持 `8004 -> 8000`；公网 `/health`、`/health/db`、供需公开列表和商机列表均返回 HTTP 200。
- 完整依赖镜像构建待服务器 apt 网络恢复后补做；当前生产热修复镜像已验证运行。
- 小程序代码不能通过服务器发布，仍需在微信开发者工具重新编译并上传体验版。

## 2026-09-07：资料与互帮互助分享卡片迁移为后端生成（本地未部署）

- 新增 `backend/app/services/share_card_renderer.py`，后端以固定 750×600 JPEG 生成名片、普通/图片资料和互帮互助三类任务分享图；名片使用圆形头像区，外部头像不抓取并使用昵称首字兜底。
- 资料分享新增后端准备接口 `/api/notes/{note_id}/share-snapshot/prepare`；互帮互助任务新增公开准备接口 `/api/scrm/mutual-help/tasks/{task_id}/share-snapshot`。前端只提交版本/指纹元数据，图片内容由服务端从已登记媒体和业务实体读取。
- 小程序资料、名片、资料库、笔记列表、资料预览、编辑器和互帮互助三类任务详情/管理页已切换为读取后端快照；任务分享缺少有效后端快照时明确禁止分享，不再回退页面截图或 Canvas。
- 分享快照增加 `renderer/backend`、尺寸、版本和历史字段；旧合集、商机、供需、回复包等本轮未确认迁移范围仍保留原 Canvas 链路。
- Dockerfile 增加 CJK 字体包，生产容器具备中文后端绘图字体；开发 mock 存储下的后端分享图也使用真实本地媒体文件，便于联调和清理。

### 验证

- 后端 Python 编译、相关小程序 JavaScript `node --check`、`git diff --check`：通过。
- 后端渲染 smoke：名片、图片资料、小程序任务、普通任务、羊毛任务均输出可读取的 750×600 JPEG。
- 分享回调对抗式检查：有效快照必须包含后端 imageUrl 和目标详情页 path；缺失/过期快照和已结束任务均返回禁止分享。
- 当前机器没有 pytest（`python3 -m pytest` 返回 `No module named pytest`），因此未执行 pytest；本轮没有部署生产，也没有上传微信体验版。

### 上线前人工验收

1. 重新构建并部署后端，确认字体、媒体目录和快照接口正常。
2. 在微信开发者工具清缓存、重新编译并上传体验版。
3. 分别验收名片、普通图片资料、无图资料、小程序任务、普通任务、羊毛任务的聊天卡片图片、详情页 path、旧版本重生成和生成失败重试；合集/商机等旧 Canvas 范围按原清单单独验收。

### 迁移收口补充

- 后端已改为自行计算分享内容指纹；前端传入的 fingerprint 不再作为渲染或复用真源，避免客户端篡改内容或导致重复重绘。
- 资料与互帮互助任务相关页面已移除隐藏 Canvas 标记和调用；旧合集、商机、供需、回复包 Canvas 范围保持不变。
- 指纹收口后的原生分享反例检查已通过：有效快照返回受管 imageUrl 与目标详情页 path，缺失/失效快照和已结束任务禁止分享。

## 2026-09-07：资料与互帮互助分享卡片后端部署生产

- 已部署到 `81.70.84.35:/home/ubuntu/teamBuy`，生产备份为 `/home/ubuntu/teambuy-backups/20260907-200336/`；未覆盖 `backend/.env`、`backend/secrets/`、数据库卷和媒体卷。
- 标准 Dockerfile 构建在 `apt-get update` 阶段网络无进度后按约定中止；随后从本次回滚镜像注入当前后端源码和 OFL 授权 Noto Sans SC 字体，生成 API、backend-worker、archive-worker 三个可回滚热修复镜像并完成切换。
- 生产容器均 healthy，端口保持 `8004 -> 8000`；本机和公网 `/health`、`/health/db` 均通过，互帮互助分享快照假 ID 路由返回预期 404，后端可导入且容器内渲染输出为 750×600 JPEG。
- 标准 Dockerfile 的完整依赖构建仍待远端 apt 网络恢复后补做；当前热修复镜像已承载线上运行。

## 2026-09-07：互帮互助任务分享卡片字号可读性收口（本地未部署）

- 按聊天卡片实际展示比例上调任务卡片标题、摘要、事实标签、无图说明和底部文案字号；固定 750×600 构图和三类任务的信息层级不变。
- 底部文案严格保留原句“不用加微信，选任务就可以互助”，没有做精简；羊毛任务继续使用“查看福利说明 · 觉得有用再参与”。
- 任务分享模板版本从 `mutual_task_backend_v2` 升到 `mutual_task_backend_v3`，旧设备上的旧分享图会被判定为过期并重新请求后端生成。
- 已通过后端 JPEG 渲染截图检查、相关 Python/JavaScript 语法检查、`git diff --check` 和原生分享回调反例检查。本次只完成本地代码，未重新部署生产、未上传微信体验版。

## 2026-09-07：任务分享卡片字号 v3 重新部署生产

- 已将 `share_card_renderer.py`、`routes_scrm.py` 的 v3 版本部署到 `81.70.84.35:/home/ubuntu/teamBuy`，生产备份为 `/home/ubuntu/teambuy-backups/20260907-213359-task-card-v3/`。
- API、backend-worker、archive-worker 使用现有生产基础镜像做范围明确的热修复并强制重建；Postgres、生产配置、密钥、媒体卷均未改动。
- 生产容器内源码哈希与本地一致，线上渲染版本为 `mutual_task_backend_v3`，底部文案为“不用加微信，选任务就可以互助”。
- 部署后 API、两个 worker、Postgres 均 healthy，本机和公网 `/health` 通过，缺失任务分享快照接口按预期返回 404。

## 2026-09-07：微信群自动化与增量同步设计讨论稿（未开发）

- 根据已验证的 AScript 无障碍模式发送文字和小程序卡片能力，整理了微信群自动化、群批次内容、首次全量扫描和后续增量同步方案。
- 新增 `docs/stage2-docs/48-wechat-group-automation-and-incremental-sync-v1.md`，明确用户、AScript 和 PC 后台的职责边界。
- 本轮没有修改业务代码、没有部署生产、没有操作手机、没有变更微信账号或发送任务。

## 2026-09-07：分享卡片可读性与名片标题修正（本地未部署）

- 按聊天卡片实际缩略比例再次上调任务、图片资料和电子名片后端渲染字号；任务卡片继续保留固定 750×600 构图和完整互助文案“不用加微信，选任务就可以互助”。
- 资料分享模板升级为 `share_card_backend_v2`，互帮互助任务模板升级为 `mutual_task_backend_v4`，旧快照不会被误当作当前版本继续分享。
- 名片原生分享标题统一为“电子名片｜姓名”，本轮示例为“电子名片｜高先生”；图片资料使用“图片资料｜标题”，不再被通用“点开查看完整资料”覆盖。
- 已完成 JavaScript/Python 语法检查、渲染尺寸 smoke、三类任务渲染检查和原生分享回调反例检查。本轮只完成本地代码，未重新部署生产、未上传微信体验版。

## 2026-09-07：双微信微信群扫描核对首轮实现

- 后端新增 `/api/automation/group-candidates/reconcile`，按微信账号接收一次扫描结果；完整覆盖才清理缺失原生微信群，部分覆盖只同步不删除。
- PC 群运营列表显示微信名，设备区显示两个账号各自的最近扫描摘要；清理只作用于 `wechat_native`，不删除小红书群记录或发送历史。
- AScript `wechat_group_inventory` 已上传手机工程，读取通讯录群聊和会话列表并分别回传两个账号；本地定向测试 16 项通过。
- 生产使用可回滚热修复镜像切换，API/两个 worker/数据库及公网健康检查通过；曾发现临时容器 `sleep` 命令被提交的问题，已回滚后从原始回滚镜像重新注入文件并确认实际启动命令恢复。
- 真机验收暂未完成：AScript 9096 在运行入口后无响应，尚未取得两个账号的最终扫描和删除计数；不把这次启动视为扫描成功。

## 2026-09-07：分享卡片 v2/v4 部署生产

- 已将资料分享模板 `share_card_backend_v2` 和互帮互助任务模板 `mutual_task_backend_v4` 部署到 `81.70.84.35:/home/ubuntu/teamBuy`。
- 部署备份为 `/home/ubuntu/teambuy-backups/20260907-232022/`；未覆盖生产 `.env`、密钥、数据库卷和媒体目录。
- API、backend-worker、archive-worker 使用当前生产镜像注入明确源码后强制重建；三个容器与 PostgreSQL 均 healthy，端口保持 `8004 -> 8000`。
- 本机、公网 `/health` 和 `/health/db` 均通过，三个应用容器内源码 SHA-256 与本地一致，并核对到资料 v2/任务 v4 模板标记。
- 小程序前端尚未重新上传；用户需在微信开发者工具清缓存、重新编译并上传体验版，真实聊天才会使用新的标题和分享图。

## 2026-09-07：AScript 资料整理助手入口菜单恢复

- 定位到 AScript 上传工具对中文工程名做 URL 编码，导致代码写入编码副本，用户实际打开的“资料整理助手”仍是旧入口。
- 将入口代码直接写入手机真实路径 `/storage/emulated/0/airscript/model/资料整理助手/__init__.py`，并从中文工程重新运行。
- 真机已显示“资料整理助手”原生功能菜单，包含“扫描微信群”；本次只验证入口显示，没有点击扫描，也没有发送或删除数据。
- 本地 `py_compile` 和 `git diff --check` 通过。

## 2026-09-07：AScript 九宫格最终真机验证

- 经过 WebWindow 兼容性测试，改为加载真实工程内的 `res/ui/launcher.html` 文件路径；内嵌 HTML 在该设备上会出现空白，文件路径方式正常。
- 中文工程“资料整理助手”已真机显示完整九宫格，第三格可见“扫描微信群”；本次仍未点击任何功能。

## 2026-09-07：分享卡片统一比例方案讨论（未开发）

- 根据最新三张聊天卡片截图，先制作统一分享图效果稿：电子名片、普通资料、图片资料和互帮互助任务共用固定 5:4 比例，营销话术固定在后端图片底部。
- 讨论中的推荐方案是后端返回完整的分享图、分享标题和详情页 path，小程序前端只负责读取并交给原生分享；微信聊天外层的应用名和开发版提示仍属于平台外壳，不纳入后端图片布局。
- 本轮只生成视觉讨论稿，没有修改业务代码、没有部署，也没有上传体验版；互帮互助任务底部文案已确认统一为“不用加微信，选任务就可以互助”。

## 2026-09-07：微信群扫描首轮双账号真机验收

- 用户在“资料整理助手”九宫格中点击第 3 格“扫描微信群”，真实中文工程中的扫描模块已成功执行，扫描结束后返回九宫格。
- 当前两个微信账号均完成完整覆盖：高同学扫描 53 个群，新增 0、更新 53、删除 0；leo 扫描 140 条账号内群记录，新增 9、更新 131、缺失 4、删除 4。
- 两个账号的会话列表停止原因为 `bottom_stable`，通讯录停止原因为 `reported_total`；没有未关联实时活码，也没有取消中的待执行任务。
- 本轮只做群台账同步和安全核对，没有发送营销内容、没有打开小程序卡片。leo 的 4 条删除受完整覆盖条件和账号边界保护，只作用于该账号对应的 `wechat_native` 群候选。

## 2026-09-08：分享卡片 v3 统一样式本地实现（待上传）

- 按已确认方案将资料分享后端模板升级为 `share_card_backend_v3`：名片底部固定为“点击查看完整名片”，头像使用 aspectFill 中心裁切后圆形遮罩。
- 电商、房源、制造业、服务业及其他无图资料共用后端绘制的通用资料插画；真实图片只替换同尺寸视觉槽位，所有资料卡保持 750×600（5:4）比例。
- 电商和房源价格改为 46px 加粗主信息；电商支持从 `variants.priceFen` 汇总价格，房源自动补充“元/月”或“万元”。非名片资料和图片资料底部统一显示“少来回解释，资料一页讲清”。
- 已同步小程序分享快照版本门禁为 `share_card_backend_v3`，旧 v2 快照会自动失效并在发送入口重新准备；本轮未部署生产、未上传微信体验版。
- 验证：Python 编译、Node 语法检查、Pillow 实际渲染和预览图视觉检查通过；系统 Python 未安装 pytest，因此 pytest 未运行。

## 2026-09-08：任务管理页三个严重 UI/交互问题收口

- 任务状态筛选“全部 / 进行中 / 已暂停 / 已结束”改为四等分独立筛选区，增加与任务卡列表的垂直间距；事件改为 `catchtap` 并校验白名单，筛选只刷新列表，不再误触任务详情。
- “查看管理 / 分享任务 / 删除任务”统一为三列等宽、同高度、同圆角的操作按钮，删除改为明确的“删除任务”按钮样式。
- “我发布的任务”和“发布任务”入口使用满宽左右布局，发布按钮固定靠右；保留现有删除、分享和详情业务处理逻辑。
- 验证：任务管理页 JS `node --check`、目标文件 `git diff --check` 和静态事件/样式引用检查通过；未使用微信开发者工具真机预览。

## 2026-09-08：分享卡后端生产部署完成

- 已将本轮后端分享渲染文件部署到 `81.70.84.35:/home/ubuntu/teamBuy`，生产备份目录为 `/home/ubuntu/teambuy-backups/20260908-012921/`；未覆盖生产 `backend/.env`、secrets、媒体目录和数据库卷。
- 因分享图需要中文字体，按规范重建 `backend`、`backend-worker`、`archive-worker` 三个镜像，切换后均为 healthy；生产端口保持 `8004 -> 8000`。
- 线上运行时已核对 `share_card_backend_v3`、中文字体、源码 SHA-256 和容器内实际生成 750×600 JPEG；本机健康检查、数据库健康检查和公网 `https://teambuy.lifelove.top/health` 均通过。
- 任务管理页属于小程序前端，本次无法通过服务器部署生效；仍需用户在微信开发者工具重新编译并上传体验版。

## 2026-09-08：微信群增量扫描与 PC 群列表收口

- AScript 九宫格第 3 格现在先选择“增量扫描（每日）”或“完整扫描（核对）”；增量模式限制通讯录/会话列表读取范围，跳过群人数检测，并以 `scanMode=incremental`、`coverage=partial` 回写后端。
- 增量真机测试已完成：leo 账号读取通讯录 10 个群、会话列表 54 个群，命中 56 个群记录；后端确认写入，未执行删除。另一微信账号本次因微信首页未出现“我”入口进入 degraded，不影响已完成账号的回写。
- PC 群运营页移除旧的“单群单卡测试”和旧群库编辑区，改为群列表：群名称、微信名、群主题、营销状态、最近发送、下一批次、今日次数、配置、记录。批次发送字段目前明确显示为未配置/未发送，不伪装成已完成。
- 线上已备份 4 个后端文件并完成临时容器热更新，公网 `/health`、数据库健康检查和 `/ops` 群列表标记验证通过；正式 Docker 重建卡在基础镜像 `apt-get update`，旧镜像已保留回滚标签 `teambuy-backend:pre-20260908-004439-wechat-group-list`。
- 本地验证：Python 编译、Node 语法检查、`git diff --check`、`backend/tests/test_automation_group_reconcile.py`（2 passed）。

## 2026-09-08：群列表筛选、列宽和批次配置第一版

- 群列表表头增加按群名、微信名、群主题/地域、营销状态、最近发送、下一批次和今日次数筛选；筛选在 PC 端即时生效，分页按筛选结果重新计算。
- 群列表列宽支持拖拽调整，并保存到当前浏览器的 `localStorage`；没有新增服务端用户偏好数据。
- 每个群的“配置”改为批次编辑窗口：支持只发文字、只发卡片或两者同时发送；支持添加批次、复制到第 N 批、删除批次，并同时保存群主题、地域、营销状态、允许内容和备注。
- 后端 `AutomationGroupCandidate` 新增 `batchContents`，扫描/upsert 会保留既有批次配置；新增批量替换接口，可按原卡片编号和/或原文字精确匹配并替换。
- 线上已备份 `/home/ubuntu/teambuy-backups/20260908-011804/`，使用带回滚标签的范围明确镜像热更新并强制重建 API、backend-worker、archive-worker；PostgreSQL、配置、密钥和媒体卷未改动。
- 验证：本地 Python 编译、Node 语法检查、`git diff --check`、自动化定向测试 3 passed；线上三个应用容器 healthy，本机与公网 `/health` 通过，`/ops` 已出现“批量替换内容”。
- 尚未完成：AScript 实际按批次领取/执行、发送成功/失败计数与历史、每日三次策略、卡片素材群目录和私聊发送。

## 2026-09-08：卡片目录与每日上限控制面

- PC 群运营页新增“小程序卡片目录”，可登记 `cardId`、小程序深链接、卡片标题和启用/停用/过期状态；目录只保存素材映射，不打开小程序、不触发发送。
- 群配置新增“每日上限”，默认 3 次，作为后续人工确认执行和发送记录的策略字段；本轮没有创建自动发送任务。
- 后端新增 `automation_card_assets` 持久化表及列表/保存接口；群候选新增 `dailySendLimit`，旧记录按默认值 3 兼容。
- 线上备份 `/home/ubuntu/teambuy-backups/20260908-020520/`，API、两个 worker、PostgreSQL healthy，公网 `/health`、页面“卡片目录/每日上限”标记和未授权 403 检查通过。
- 本地自动化定向测试 4 passed；AScript 发送器仍保持“领取/校验后停止”的安全边界，没有实现非官方微信自动群发或私聊发送。

### 2026-09-08 分享卡片 01:47 线上核验

- 针对用户反馈“01:47 发送后仍像老款”的时间窗口，生产日志显示 10 次资料分享快照准备请求全部返回 200；对应资料的 `shareSnapshot.styleId` 均为 `share_card_backend_v3`，状态为 `ready`，`sourceRevision` 与资料当前 revision 一致。
- 线上实际打开名片、房源快照均为 750×600 的后端 v3 图片；因此旧资料本身可以使用新模板，不需要重新创建资料。
- 历史微信群消息中的旧图片不会被服务端替换；必须从小程序当前资料/名片的“发客户”入口重新发送。合集/展示页仍使用 `share_card_v12` 的独立分享链路，不能与资料 v3 结论混用。
- 本次为只读核验，没有修改业务代码、数据库或再次部署。

### 2026-09-08 分享卡发送链路进一步定位

- 用户确认是从当前入口重新发送后，继续检查发现 `miniprogram/pages/library/index.js` 的 `getPreparedLibraryShareImage()` 只校验 `noteId` 和 `sourceRevision`，没有校验 `styleId`；`prepareShare()` 与 `onShareAppMessage()` 都会优先使用这份页面内缓存图片。
- 线上 10 条资料的 `shareSnapshotHistory` 均能看到旧版 `share_card_v10/v6`，当前快照已是 `share_card_backend_v3`。因此资料内容不变时，资料库页面仍可能把同 revision 的旧图当成可发送图片。
- 该问题已定位为小程序资料库发送链路的版本门禁缺失；本轮只完成定位，尚未修改或上传小程序。

### 2026-09-08 分享卡资料库旧图修复

- `miniprogram/pages/library/index.js` 已修复：`shareImages` 命中前同时校验 `noteId`、`sourceRevision`、`styleId` 和 fingerprint；旧版本缓存不再覆盖服务端当前快照。
- 新写入的页面缓存记录显式保存 `styleId`，资料库发送前和 `onShareAppMessage` 取图均经过同一版本门禁。
- 验证：资料库页 JS、分享插件 JS `node --check` 通过，目标文件 `git diff --check` 通过。
- 尚未上传小程序体验版；该修复只需重新编译并上传小程序，不需要再次部署后端。

## 2026-09-08 分享卡 v4 样式与字段收口

- 资料分享后端模板升级为 `share_card_backend_v4`，旧 v3 快照会因 styleId 不匹配而重新准备；固定尺寸仍为 750×600。
- 电子名片静态图补齐联系方式：普通名片读取实际电话/微信/邮箱；启用商机合作的名片不泄露真实联系方式，改为显示“联系方式已保护 · 详情页查看”。
- 房源/电商微信外层分享标题改为“房源资料｜名称”和“电商资料｜名称”，价格、户型、面积继续由后端图片主体承载，避免外层标题重复。
- 已确认资料列表与资料库左侧缩略图均使用 `aspectFill`；本轮未修改该部分。
- 本地 Python 编译、Node 语法检查、分享标题回调样例和 `git diff --check` 通过；系统 Python/虚拟环境未安装可运行的 pytest/Pillow 依赖，后端渲染 pytest 未执行。尚未部署生产、未上传小程序体验版。

### 2026-09-08 分享卡 v4 生产部署

- 已将后端资料分享渲染器 `share_card_backend_v4` 部署到生产，API 当前镜像为 `teambuy-backend:deploy-20260907-185417`，容器内源码 SHA-256 与本地变更一致。
- 部署前已检查磁盘和 Docker 状态；生产备份位于 `/home/ubuntu/teambuy-backups/20260907-185342-share-card-v4/`，只备份本次 Compose/渲染器并保留 API、backend-worker、archive-worker 回滚标签，未改动 `backend/.env`、secrets、数据库卷和媒体目录。
- API、backend-worker、archive-worker、PostgreSQL 均 healthy；本机 `127.0.0.1:8004/health`、数据库健康检查和公网 `https://teambuy.lifelove.top/health` 均通过，公网返回 200。
- 本地使用 Python 3.13 `.venv313` 执行 `PYTHONPATH=.:backend ./.venv313/bin/python -m pytest -q backend/tests`，结果为 `385 passed`。
- 小程序前端尚未上传体验版；手机端要使用资料库版本门禁和 v4 分享入口，仍需在微信开发者工具重新编译并上传体验版。已发送的微信群历史卡片不会自动更新。

### 2026-09-08 分享卡外层标题补充

- 线上核对“你好”资料确认后端当前快照为 `share_card_backend_v4`，截图中的新版无图主体已经生效；剩余差异是小程序外层标题没有给普通文字资料加“文字资料｜”前缀。
- 已修改 `miniprogram/plugins/share-snapshot/index.js`：普通资料标题统一为“文字资料｜名称｜点开查看完整资料”，与图片资料、房源、电商和电子名片保持同一规则。
- 验证：`node --check`、文字/图片/房源/电商四类标题断言和 `git diff --check` 通过；本次没有后端代码变化，不重复部署 API。
- 微信原生卡片底部的“未发布的小程序·体验版”由当前体验包环境显示，后端不能隐藏；待重新上传小程序体验版后再人工确认标题和原生尾部表现。

### 2026-09-08 分享卡 v5 统一图层与正式插画

- 按最终图层规范重做资料卡：图 1 是微信原生外壳，图 2 是后端完整 750×600（5:4）图片；类型、标题、价格、说明和底部营销条全部属于图 2。
- 无真实主图时，图 2 左侧主视觉位使用正式透明插画 `backend/app/static/share-card-assets/universal-info-artwork.png`；不再使用 Pillow 简笔画，也不把整张效果图嵌套进卡片。
- 普通资料、图片资料、房源、电商、服务和制造业统一使用“类型 + 有图/无图 + 左视觉/右信息 + 底部营销条”骨架；普通资料外层标题统一为“普通资料｜名称｜点开查看完整资料”。
- 资料分享版本升级为 `share_card_backend_v5`，旧 v4 快照进入过期状态并在重新进入发送入口时生成新图。
- 定向渲染测试 6 passed；全量测试首次为 385 passed、1 个既有二维码 OpenCV 识别偶发失败，单独重试通过。Node 语法和差异检查通过。
- 已部署生产：API 与 backend-worker 切换到 v5，正式插画已核对 SHA-256；API、两个 worker、PostgreSQL healthy，本机/公网 `/health` 和生产渲染 smoke 通过。备份位于 `/home/ubuntu/teambuy-backups/20260908-033806-share-card-v5/`。
- 待负责人操作：重新编译上传小程序体验版；服务器部署不能替换手机端外层标题，历史微信群卡片也不会自动更新。

## 2026-09-08：Mac + ESP32 + AScript 三步诊断

- Mac `/dev/cu.usbmodem101` 只读探测通过：芯片为 ESP32-C3，MAC `10:20:BA:DC:5D:90`；未刷写固件。
- AScript 4.0.01 手机端已连接，`资料整理助手` 可由 Mac 启动；当前运行模式实际为 `HID控件模式`，不是“数字助理模式”。
- `plug.load("esp32:4.7.0")` 与历史版 `4.1.2` 均返回成功，但 `import esp32` 分别报 `bad magic number`；问题位于手机端插件加载/运行时兼容，不是 Mac 串口或 ESP32 芯片识别。
- 当前手机蓝牙运行权限为未授权，且项目代码仍明确使用 `ascript_native`、暂停 ESP32；因此本轮未执行 HID 点击/文字输入，避免把未验证链路用于微信。

## 2026-09-08：分享卡标题长度与名片联系方式优化

- 小程序所有资料分享标题统一限制为最多 40 个 Unicode 字符，超出部分截为 37 个字符并追加 `...`；普通资料、图片、房源、电商、服务和名片入口均复用同一规则。
- 电子名片外层标题优先读取名片结构化姓名/昵称，再回退到个人资料昵称；不再依赖固定示例姓名。
- 商机合作名片的公开静态图不展示完整电话，改为显示脱敏电话（例如 `电话 138****0000 · 详情页查看`），真实号码仍只在详情解锁后返回。
- 资料分享版本升级为 `share_card_backend_v6`，避免同一 revision 继续复用 v5 旧图；本轮仅完成本地代码和测试，尚未部署生产或上传小程序体验版。

## 2026-09-08：区分名片所有者分享与商机公开名片

- 修正名片联系方式边界：名片所有者从“发客户/发名片”准备自己的分享图时，后端显式使用所有者分享上下文，显示完整电话、微信或邮箱。
- 商机合作列表/详情仍按公开权限脱敏；若未来存在非所有者生成公开名片图的入口，默认继续显示脱敏电话或保护提示。
- 资料分享版本仍为 `share_card_backend_v6`；定向渲染测试 6 passed，Python 编译、相关小程序 JS 语法检查和 `git diff --check` 通过。本轮尚未部署生产或上传体验版。

## 2026-09-08 分享卡 v6 生产部署

- 已将 `share_card_backend_v6` 和名片所有者完整联系方式渲染修复部署到生产；API 镜像为 `teambuy-backend:deploy-20260908-041355`，backend-worker 同步更新。
- 部署前已备份生产 Compose、API 当前渲染器、`app_service.py`、正式无图插画和容器信息；备份位于 `/home/ubuntu/teambuy-backups/20260908-041355-share-card-v6/`，并保留 `rollback-20260908-041355` 镜像标签。
- 未改动生产 `backend/.env`、`backend/secrets/`、数据库容器、数据库卷和媒体目录。
- API、backend-worker、archive-worker、PostgreSQL 均运行；本机 API `/health`、`/health/db` 和公网 `https://teambuy.lifelove.top/health` 均返回 200，数据库连接正常。
- 生产容器内已核对 `share_card_backend_v6`、`include_owner_contacts=True` 和正式插画 SHA-256；小程序前端仍需重新编译上传体验版，才能验证手机端外层标题和重新发送流程。

## 2026-09-08 分享卡 v7：标题、长字段与字号优化

- 原生微信分享外层标题统一收紧到最多 20 个 Unicode 字符，超出使用 `...`，避免房源等长标题在图 1 中堆叠成多行。
- 房源/商品等后端图片右侧信息区的标题、摘要和事实字段增加长度门禁；长地址等内容最多保留 20 个字符并显示省略号。
- 后端分享图关键文字整体放大：名片姓名、电话、身份信息、普通资料标题/说明、任务文字和底部文案同步提升字号，并通过图片级样例检查确认未越界。
- 修复“我的 → 发名片”空白：摘要接口和个人页分享逻辑统一把分享图 URL 规范化为完整 HTTPS 地址；资料库发送链路保持不变。
- 分享版本升级为 `share_card_backend_v7`，用于强制重新生成旧 v6 快照；本轮仅完成本地代码与测试，尚未部署生产。

## 2026-09-08 分享卡 v8：底部营销文案可读性优化

- 将普通资料/图片/房源/电商/服务/制造业等资料卡底部“少来回解释，资料一页讲清”和互助任务底部“不用加微信，选任务就可以互助”统一放大到 38 号字；名片底部操作文案同步保持同一可读性基线。
- 分享图样式版本升级为 `share_card_backend_v8`，确保 v7 旧快照不会继续复用，重新准备分享图后才使用新字号。
- Python 3.13 定向渲染测试 6 passed；相关小程序 JS 语法检查、Python 编译检查和 `git diff --check` 均通过；图片级样例确认两类营销文案未超出固定 750×600 图层。
- v8 后端已部署生产，部署批次为 `20260908-051050`；生产备份位于 `/home/ubuntu/teambuy-backups/20260908-051050-share-card-v8/`，并保留 API、backend-worker、archive-worker 的回滚镜像标签。
- 生产 API、两个 worker、数据库均 healthy，8004 端口保持不变，本机/公网健康检查和生产真实渲染 smoke test 通过；小程序前端仍需重新编译上传体验版。

## 2026-09-08 分享卡 v8：电子名片纵向布局修复

- 电子名片图 2 的职位/服务、联系电话和“我能提供”区域整体上移，给联系方式与下方色块留出明确间距，避免文字被遮挡。
- 保持 `share_card_backend_v8` 不变，仅增加后端渲染修订 `business_card_layout_v2` 并纳入服务端指纹；已有 v8 快照在下次准备分享图时会自动重生成，因此不需要修改小程序前端。
- 后端 API、backend-worker、archive-worker 已部署，批次为 `20260908-052803`；生产健康检查、数据库检查和 750×600 名片渲染 smoke test 通过。
- 生产备份位于 `/home/ubuntu/teambuy-backups/20260908-052803-share-card-layout-v2/`，未改动 `.env`、数据库和媒体目录。

## 2026-09-08 分享卡 v8：旧名片快照失效保护已生产部署

- 发现仅调整后端坐标仍可能复用已有 v8 静态图：小程序按 `sourceRevision/styleId` 判断快照可用，不会仅因服务端指纹变化再次请求准备接口。
- 后端新增 `rendererRevision=business_card_layout_v2`，并在名片摘要、个人资料列表和公开资料配置返回前移除旧修订的 v8 快照，确保再次进入发送流程时重新生成新版名片图。
- API、backend-worker、archive-worker 已统一切换到部署批次 `20260908-053640`；三套镜像中的渲染器和 `app_service.py` 哈希一致。
- 生产 `/health`、`/health/db`、公网 `/health`、8004 端口和生产容器健康检查通过；近期后端日志未发现 error/traceback/exception。
- 生产备份位于 `/home/ubuntu/teambuy-backups/20260908-053640-share-card-layout-v3/`，保留对应回滚镜像，未改动 `.env`、数据库和媒体目录。

## 2026-09-08 分享卡 v8：电子名片间距与长内容优化（本地完成）

- 电子名片职位/服务与电话继续上移，联系方式和下方“我能提供”信息块之间增加安全间距；信息块顶部下移，避免任何一行文字被背景色块遮挡。
- “我能提供”内容增加 20 字硬上限，超出时使用三个半角句点 `...`，不再让长内容在固定卡片中堆叠。
- 渲染修订升级为 `business_card_layout_v3`，保持 `share_card_backend_v8` 不变以兼容现有小程序；本轮只改后端，尚未部署生产。
- Python 3.13 分享卡测试 7 passed，Python 编译和 `git diff --check` 通过；本地 750×600 样例确认电话与信息块分离、长内容正确省略。

## 2026-09-08 分享卡 v8：电子名片服务行动态排版（本地完成）

- 修正上一版“企业服务”固定坐标过高导致与姓名重叠的问题：姓名下方按实际内容依次排布职位/公司、服务范围和联系电话，各行自动保持固定行距。
- 没有服务范围时，联系电话自动紧跟职位/公司；有服务范围时，联系电话顺延一行，避免出现过大空隙或上下重叠。
- 渲染修订升级为 `business_card_layout_v4`，继续保持 `share_card_backend_v8` 和后端-only 方案；本轮尚未部署生产。
- 本地 750×600 样例分别验证了“企业服务作为职位”和“企业服务作为服务范围”两种数据形态，均无重叠。

## 2026-09-08 分享卡 v8：电子名片服务行动态排版已部署

- 已将 `business_card_layout_v4` 部署到生产，API、backend-worker、archive-worker 三套镜像统一更新，仍保持 `share_card_backend_v8` 兼容标识。
- 生产验证通过：容器、数据库、8004 端口、本机/公网健康检查正常；生产渲染 smoke test 输出 750×600 JPEG。
- 生产备份位于 `/home/ubuntu/teambuy-backups/20260908-060255-share-card-layout-v4/`，回滚标签为 `rollback-20260908-060255`；未改动 `.env`、数据库和媒体目录。
- 用户需从原有“发名片”入口重新准备并发送，新卡片才会使用动态排版；历史微信群图片不会被替换。

## 2026-09-08 互帮互助任务入口补齐（小程序前端本地完成）

- 普通任务和羊毛任务的“任务入口（选填）”默认展开，不再要求用户先点击“展开”才看到链接输入框；仍可手动收起。
- 入口解析继续区分 `#小程序://` 和 HTTPS 网页：目标小程序优先调用 `wx.navigateToMiniProgram`；公众号、小红书、淘宝、京东/拼多多网页 URL 在无法内嵌时显示“复制到浏览器”。
- 目标小程序跳转失败时增加“复制入口”兜底；已配置业务域名的网页容器加载失败时增加“复制到浏览器”按钮，不再只有失败提示。
- 已通过相关 JavaScript `node --check`、小程序 JSON 解析、入口解析断言和 `git diff --check`；本轮未改后端，需用户在微信开发者工具重新编译上传体验版后真机验收。

## 2026-09-08：普通任务分享失败与管理详情流程修复（本地完成）

- 根因确认：普通任务分享图渲染器从任务内容块取第一张图片后，未判断图片是否是服务端可读取的托管媒体；公众号截图外链、临时地址或失效地址会让 `_load_image()` 抛出渲染错误，分享快照接口返回失败，管理列表因此显示“分享暂时失败”。任务入口里的公众号网页链接不是这个失败的直接原因。
- 修复：`render_mutual_task()` 只使用当前存储服务可读取的托管图片；没有可用托管图时自动使用任务无图视觉区，外链图片不再阻断分享图生成。新增“外链图片 + 公众号入口”的真实接口回归用例。
- 管理详情按执行流程重排为“任务入口 → 任务说明 → 完成标准 → 参与记录/验收 → 任务评论”，入口和任务内容不再埋在评论与参与记录之后；执行者详情原本已经按入口优先排列。
- 验证：`.venv313/bin/python -m pytest -q backend/tests/test_share_card_renderer.py backend/tests/test_mutual_help_points.py backend/tests/test_my_tools_cross_device_and_gates.py`（28 passed）、全量小程序 JS `node --check`、84 个小程序 JSON 解析、Python 编译和 `git diff --check` 均通过。
- 本轮未部署生产、未上传微信体验版。公众号 `mp.weixin.qq.com` 不属于当前小程序已配置的 web-view 业务域名，仍按微信限制提供“复制到浏览器”兜底；要内嵌打开需先在微信后台配置业务域名并真机验收。

## 2026-09-08：AScript + ESP32 HID 微信单群单条测试诊断

- 已通过 AScript MCP 连接 Android-237（192.168.1.237:9096），读取设备状态：Android 15、AScript 4.0.03、`run_mode=HID控件模式`；设备蓝牙运行权限当前为未授权。
- 已尝试截图留证，但设备 `screen_capture` 权限为未授权，AScript 返回空 Bitmap；本轮没有可交付的设备截图文件。
- 已启动设备上的 `hid-test`。读取到的工程仍是 `plug.load("esp32")`、`BleDevice()` 和一次 `ble.click()` 的最小探针；运行期间没有收集到日志，随后已停止工程。
- `plug.load("esp32:4.7.0")` 返回 true 但 `from esp32 import BleDevice` 报 `bad magic number in 'esp32': b'H8tX'`；历史版 4.1.2 同样导入失败，错误为 `b'gPGQ'`。
- 依据官方插件文档调用一次无害的 `action.Hid.click(474,177)` 做点击通道验证，但返回 `unexpected url: :8080/api/click`，没有发出点击。
- 本次未打开微信、未定位“自动化测试群”、未输入 `HID_TEST_20260908`、未点击发送、未进入卡片阶段；没有修改微信、注入进程、使用私有协议或读取 token/session/key。
- 详细逐步日志见 `docs/qa/AScript_ESP32_微信单群单条测试_20260908_诊断报告.md`。当前不输出 `READY_TO_SEND_TEXT` / `READY_TO_SEND_CARD`；恢复 ESP32 插件兼容性或补齐 HID 主机配置后再继续。

### 证据纠正：`ble.click()` 返回 `None` 属于正常结果

- 用户提供的 AScript IDE 原始日志显示：`esp32` 模块成功导入、`AS_4.8_905DDCBA2010` 连接成功、`已连接`，随后 `ble.click(394,2116)` 输出 `点击 None`。
- `BleDevice.click()` 是无返回值动作函数，`None` 表示没有返回对象，不表示点击失败；该次运行至少证明 ESP32 HID 点击调用未抛异常。
- 之前将 `None` 误读成失败，并将官方 `BleDevice` 路径与内置 `action.Hid` 空主机错误混为同一链路，现予以纠正。剩余验证项是点击前后手机 UI 是否实际变化。

## 2026-09-08：互帮互助任务公众号文章入口修复（本地完成）

- 普通任务和羊毛任务的 HTTPS 入口增加 `mp.weixin.qq.com` 专用识别，任务详情和发布者管理详情优先调用 `wx.openOfficialAccountArticle`。
- 官方能力不可用或打开失败时复制公众号文章链接；小程序短链接仍使用 `wx.navigateToMiniProgram`，自有业务域名仍使用 `web-view`，其他外部网页仍复制到浏览器。
- 验证：公众号文章/自有 H5/京东网页/小程序短链 4 类入口解析断言通过；全量小程序 JS 语法检查、JSON 解析通过；后端互帮互助回归测试 19 passed。
- 本轮只修改小程序前端，尚未上传微信体验版；需要重新编译上传后在真机验证公众号 API 成功和失败兜底。
