...
如果本次确实新增运行依赖，而完整构建在一次有证据的网络重试后仍无法完成，不能退回“只覆盖源码”的热修复。可使用已有生产基础镜像制作依赖完整的发布镜像：从受控网络下载并校验 wheel，显式安装新增依赖，覆盖已校验源码，恢复每个服务的正确 `CMD`，再用 Compose 的生产环境变量和 PostgreSQL 网络执行一次临时 smoke test；只有 smoke test、镜像 import、服务健康检查全部通过后才允许切换。该路径是完整构建受阻时的受控例外，不是默认发布方式。
