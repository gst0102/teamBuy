...
2. 重点验收资源库搜索、分类 chip、标签 chip、资源卡片「详情 / 访问 / 复制 / 编辑」入口。
3. 继续验收本地可跑链路：手动添加资源 -> 编辑 -> 发布查看 -> 拨号/复制/实名接龙 -> 管理页 -> 一键复用。
4. 继续验收 mock 企业微信链路：发给客服 mock 导入 -> 待认领 -> 认领编辑 -> 发布查看。
5. 企业微信认证问题解决后，优先排查真实 `sync_msg` 的 `48002 api forbidden`：
...
7. 继续测试真实链接、图片、视频、位置和微信笔记。
8. 真实链路跑通后，再补充 QA 验收报告，不要提前标记上线通过。
