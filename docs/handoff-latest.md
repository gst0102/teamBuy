...
1. 先把当前小程序在微信开发者工具中跑起来，完成登录、tabBar、首页、资源库、发给客服、访问记录、我的页人工验收。
2. 继续验收 mock 旧链路：发给客服 mock 导入 -> 待认领 -> 认领编辑 -> 发布查看 -> 拨号/复制/实名接龙 -> 管理页 -> 一键复用。
3. 优先解决企业微信真实 `sync_msg` 的 `48002 api forbidden`：
...
5. 继续测试真实链接、图片、视频、位置和微信笔记。
6. 真实链路跑通后，再补充 QA 验收报告，不要提前标记上线通过。
