...
- OCR 入口：第一版后端接口为 `POST /api/ocr/image-to-note`，小程序“我的笔记”页提供“图片识别”。OCR 只把图片转为文字和媒体引用，写入 `structuredData.ocr` 后统一进入 `ContentObject.sourceType=image_ocr -> content-to-note -> UserNote`；引擎通过 `OCR_PROVIDER=auto/paddle/tesseract/mock` 配置，未配置时保留图片资料并允许手动补充。
