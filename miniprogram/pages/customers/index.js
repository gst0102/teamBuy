...
function filterCustomers(customers, filter) {
  ...
  return customers.filter((item) => (item.intentLevel || "待判断") === filter);
}
...
Page({
  ...
  async loadCustomers() {
    ...
    try {
      ...
      this.setData({
        customers,
        filteredCustomers: filterCustomers(customers, this.data.activeIntent),
        summary: {
  ...
  handleIntentFilterChange(event) {
    ...
    this.setData({
      activeIntent,
      filteredCustomers: filterCustomers(this.data.customers, activeIntent)
    });
  },
