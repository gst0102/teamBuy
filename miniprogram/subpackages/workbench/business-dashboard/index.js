...
Page({
  ...
  async loadDashboard(ownerUserId) {
    ...
    try {
      const membershipRes = await api.fetchMembership(ownerUserId);
      const accessState = getCustomerAccessState((membershipRes && membershipRes.data) || {});
      if (accessState === "unknown") {
        this.applyDashboardView(normalizeDashboard({}, this.data.mode), DEFAULT_VISITOR_FILTER, { errorText: "客户信息暂时无法读取，请重试" });
        return;
      }
      if (accessState === "disabled") {
        this.applyDashboardView(normalizeDashboard({}, this.data.mode), DEFAULT_VISITOR_FILTER, { errorText: "客户信号暂未开放" });
        return;
      }
  ...
  handleOpenShowcaseAnalytics(event) {
    ...
    wx.navigateTo({ url: `/subpackages/workbench/resource-analytics/index?entityType=showcase&showcaseId=${showcaseId}` });
  },
  handleOpenLeads() {
  ...
  handleOpenCustomerAction(event) {
    ...
    if (action.targetType === "lead" && action.leadReminderId) {
      wx.navigateTo({ url: `/pages/customer-detail/index?leadId=${encodeURIComponent(action.leadReminderId)}` });
      return;
    }
    if (action.noteId) {
      wx.navigateTo({ url: `/subpackages/workbench/note-actions/index?id=${action.noteId}` });
    }
  },
  handleOpenPriorityContact(event) {
    ...
    if (item.targetType === "lead" && item.leadReminderId) {
      wx.navigateTo({ url: `/pages/customer-detail/index?leadId=${encodeURIComponent(item.leadReminderId)}` });
      return;
    }
    if (item.noteId) {
      wx.navigateTo({ url: `/subpackages/workbench/note-actions/index?id=${item.noteId}` });
    }
  },
  ...
  handleOpenSelectedVisitorTarget() {
    ...
    if (visitor.leadReminderId) {
      wx.navigateTo({ url: `/pages/customer-detail/index?leadId=${encodeURIComponent(visitor.leadReminderId)}` });
      return;
    }
    if (visitor.noteId) {
      wx.navigateTo({ url: `/subpackages/workbench/note-actions/index?id=${visitor.noteId}` });
      return;
    }
