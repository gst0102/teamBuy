...
Page({
  ...
  async loadCard() {
    try {
      ...
      if (currentUser && card.ownerUserId === currentUser.id && card.sourceNoteId) {
        wx.redirectTo({ url: editorPathForCardType(card.cardType || ((card.visibilityConfig || {}).cardType) || "text_note", card.sourceNoteId) });
        return;
      }
  ...
  async handlePublish() {
    ...
    try {
      ...
      wx.navigateTo({ url: `/pages/note-preview/index?id=${encodeURIComponent((this.data.card && this.data.card.sourceNoteId) || this.data.cardId)}` });
    } catch (error) {
