...
const PLAIN_NOTE_SYSTEM_TEXT = new Set(["未命名笔记", "手动创建的普通笔记", "手动创建，可继续补充内容。"]);

function cleanPlainNoteText(value) {
  const result = String(value || "").trim();
  return PLAIN_NOTE_SYSTEM_TEXT.has(result) ? "" : result;
}
...
function readLastContactPhone() {
  return lastContactPhone;
}

function rememberContactPhone(value) {
  const phone = extractPhone(value);
  if (!phone) return;
  // A phone number is sensitive contact data. Keep the editor convenience
  // value in process memory only; never persist it in wx storage.
...
function buildMediaItems(form) {
  ...
  media.forEach((item, index) => {
    ...
    items.push({
      ...
      label: item.type === "image" && item.url === form.coverUrl ? "封面" : item.type === "video" ? "视频" : item.type === "pdf" ? "PDF" : item.type === "link" ? "链接" : "图片"
    });
  });
  return items;
}

function buildContentBlocks(form) {
  const blocks = [];
  const body = String((form && form.body) || "").trim();
  if (body) blocks.push({ id: "block_text", type: "text", text: body, sortOrder: 0 });
  (Array.isArray(form && form.media) ? form.media : [])
    .filter((item) => item && item.url && ["image", "pdf", "link"].includes(item.type))
    .forEach((item, index) => blocks.push({
      ...item,
      id: item.id || `block_${item.type}_${index}`,
...
function buildMapAddressCandidates(structuredData) {
  ...
  const simpleCommunity = fields.community.replace(/\d+\s*(?:户型|房源|栋|号|室).*$/u, "").trim();
  const candidates = [
    ...rawCandidates,
    ...rawCandidates.map(simplifyMapAddress),
    simpleCommunity
...
Page({
  ...
  onLoad(options) {
    ...
    this.setData({
      ...
      reviewContacts: options.review === "contacts"
    });
  ...
  async loadNote() {
    ...
    try {
      // Archive imports are written by a separate worker. A cached note can
      // therefore predate the media transfer even when the library card has
      // already received the fresh cover URL; refresh the detail before
      // deciding which attachments to render.
      const res = await api.fetchNote(noteId, user.id, { force: true });
      const note = res.data || {};
  ...
  applyLoadedNote(note) {
    const config = note.visibilityConfig || {};
    const cardType = config.cardType || (config.contentMode === "bookmark" ? "link" : "text_note");
    const { editorPathForCardType, GENERAL_EDITOR_CARD_TYPES } = require("../../../utils/resource-navigation");
    if (!GENERAL_EDITOR_CARD_TYPES.has(cardType)) {
    ...
    delete effectiveConfig.marketingRoute;
    ...
    const form = {
      title: cardType === "text_note" ? cleanPlainNoteText(note.title) : note.title || "",
      summary: cardType === "text_note" ? cleanPlainNoteText(note.summary) : note.summary || "",
      body: cardType === "text_note" ? cleanPlainNoteText(note.body) : note.body || "",
      contentBlocks: Array.isArray(note.contentBlocks) ? note.contentBlocks : [],
    ...
    const isImageNote = isImagePrimaryNote({ ...note, visibilityConfig: effectiveConfig });
    const imageCaption = isImageNote
      ? cleanImagePrimaryText(form.body || structuredData.rawText || (structuredData.ocr || {}).text || "")
      : "";
    const imageContentItems = mediaItems.filter((item) => item.type === "image");
    const attachmentMediaItems = isImageNote
      ? mediaItems.filter((item) => item.type !== "image")
      : mediaItems;
    if (isImageNote) {
      form.title = imagePrimaryTitle(note, imageCaption);
      form.body = imageCaption;
      form.summary = cleanImagePrimaryText(form.summary) || imageCaption.slice(0, 120);
    }
    ...
    const isPlainNote = cardType === "text_note" && !isImageNote && !isBookmark && !miniappInfo.visible && !ocrInfo.visible && !suggestionButtons.length;
    const showOperationalControls = !isPlainNote || this.data.plainNoteOpsOpen;
    const contactCandidates = buildContactCandidates([form.title, form.summary, form.body], {
      publicPhone: form.phone,
      privatePhones: normalizePhoneList((effectiveConfig.privateData || {}).contactPhones || []),
    ...
    this.setData({
      ...
      attachmentMediaItems,
      isPlainNote,
      plainAttachmentsOpen: isPlainNote ? mediaItems.length > 0 : this.data.plainAttachmentsOpen,
      plainBodyCount: isPlainNote ? String(form.body || "").length : 0,
      ...
      displayTitle: isImageNote ? form.title : buildDisplayTitle(cardType, form, structuredData),
      displaySubtitle: isImageNote ? (imageCaption !== form.title ? imageCaption : "") : buildDisplaySubtitle(cardType, form, structuredData),
  ...
  updateShareMenu() {
    const shareState = getNoteShareSnapshotState({ ...(this.data.form || {}), revision: this.data.revision }, this.data.user ? this.data.user.id : "", this.data.user || {});
    const snapshot = shareState.snapshot;
    const ready = Boolean(
      snapshot
  ...
  handleInput(event) {
    ...
    const contactCandidates = buildContactCandidates([form.title, form.summary, form.body], {
      publicPhone: form.phone,
      privatePhones: normalizePhoneList((config.privateData || {}).contactPhones || []),
      previous: this.data.contactCandidates
    });
    this.setData({
      form,
      [`form.${key}`]: value,
      plainBodyCount: this.data.isPlainNote && key === "body" ? String(value || "").length : this.data.plainBodyCount,
  ...
  handleContactDecision(event) {
    ...
    let privatePhones = normalizePhoneList(privateData.contactPhones || []).filter((item) => item !== phone);
    let candidates = (this.data.contactCandidates || []).map((item) => item);
    if (decision === "public") {
      form.phone = phone;
      candidates = candidates.map((item) => ({
        ...item,
        status: item.phone === phone ? "public" : item.status === "public" ? "pending" : item.status,
        statusLabel: item.phone === phone ? "公开电话" : item.status === "public" ? "待处理" : item.statusLabel
      }));
    } else {
      form = { ...form, ...removePhoneFromFields({ title: form.title, summary: form.summary, body: form.body }, phone) };
      if (form.phone === phone) form.phone = "";
      if (decision === "private") privatePhones.push(phone);
      candidates = candidates.filter((item) => item.phone !== phone);
    }
    if (privatePhones.length) privateData.contactPhones = Array.from(new Set(privatePhones));
    else delete privateData.contactPhones;
    form.visibilityConfig = { ...currentConfig, privateData };
    this.setData({ form, contactCandidates: candidates, plainBodyCount: String(form.body || "").length }, () => {
      const next = buildContactCandidates([form.title, form.summary, form.body], {
        publicPhone: form.phone,
        privatePhones,
        previous: this.data.contactCandidates
      });
      this.setData({ contactCandidates: next }, () => wx.showToast({
        title: decision === "public" ? "已设为公开电话" : decision === "private" ? "已移入私密资料" : "已从正文移除",
        icon: "success"
      }));
    });
  },
  handleTogglePlainAttachments() {
    this.setData({ plainAttachmentsOpen: !this.data.plainAttachmentsOpen });
  },
  handleOpenPlainAttachment(event) {
    const item = this.data.mediaItems[Number(event.currentTarget.dataset.index)];
    if (!item || !item.url) return;
    if (item.type === "image") {
      const urls = (this.data.mediaItems || []).filter((row) => row.type === "image").map((row) => row.displayUrl || row.url).filter(Boolean);
      wx.previewImage({ current: item.displayUrl || item.url, urls });
      return;
    }
    if (item.type === "link") {
      wx.setClipboardData({ data: item.url, success: () => wx.showToast({ title: "链接已复制", icon: "success" }) });
      return;
    }
    wx.downloadFile({
      url: item.url,
  ...
  async handleAddToShowcase() {
    ...
    wx.navigateTo({ url: `/subpackages/workbench/showcase-edit/index?mode=notes&noteId=${encodeURIComponent(noteId)}` });
  },
  ...
  handlePosterEntry() {
    wx.navigateTo({ url: `/subpackages/workbench/note-poster/index?id=${this.data.noteId}` });
  },
  handlePreviewPage() {
    wx.navigateTo({ url: `/pages/note-preview/index?id=${this.data.noteId}` });
  },
  async handlePlainPreview() {
    if (this.data.saving) return;
    this.setData({ saving: true });
    try {
      await this.handleSaveOnly();
      wx.navigateTo({ url: `/pages/note-preview/index?id=${this.data.noteId}&preview=1` });
    } catch (error) {
      wx.showToast({ title: error.detail || error.message || "保存失败", icon: "none" });
    } finally {
      this.setData({ saving: false });
  ...
  handleOpenCustomerActions() {
    ...
    wx.navigateTo({ url: `/subpackages/workbench/note-actions/index?id=${this.data.noteId}` });
  },
  handleOpenBusinessCardStudio() {
    if (!this.data.noteId) return;
    wx.navigateTo({ url: `/subpackages/workbench/business-card-studio/index?id=${this.data.noteId}` });
  },
  handleOpenServiceOfferStudio() {
    if (!this.data.noteId) return;
    wx.navigateTo({ url: `/subpackages/workbench/service-offer-studio/index?id=${this.data.noteId}` });
  },
  handleOpenMessages() {
  ...
  chooseNotePdf() {
    wx.chooseMessageFile({
      ...
      type: "file",
      extension: ["pdf"],
      success: ({ tempFiles = [] }) => this.uploadNoteMedia(tempFiles.map((item) => item.path).filter(Boolean), "pdf")
    });
  },
  addNoteLink() {
    wx.showModal({
      ...
      success: async ({ confirm, content }) => {
        ...
        const media = [...(this.data.form.media || []), { id: `link_${Date.now()}`, type: "link", title: url, url, sortOrder: (this.data.form.media || []).length, source: "manual", status: "ready" }];
        this.applyMediaState({ ...this.data.form, media });
        await this.handleSaveOnly();
        wx.showToast({ title: "链接已添加", icon: "success" });
      }
    });
  },
  chooseNoteImages() {
    const imageCount = (this.data.form.media || []).filter((item) => item && item.type === "image").length;
    const count = Math.max(0, 9 - imageCount);
    if (!count) { wx.showToast({ title: "图片数量已达上限", icon: "none" }); return; }
    if (typeof wx.chooseImage === "function") {
      wx.chooseImage({
        count,
        sourceType: ["album", "camera"],
        success: ({ tempFilePaths = [], tempFiles = [] }) => this.uploadNoteMedia(tempFilePaths.length ? tempFilePaths : tempFiles.map((file) => file.path).filter(Boolean), "image")
      });
      return;
    }
    wx.chooseMedia({ count, mediaType: ["image"], sourceType: ["album", "camera"], success: ({ tempFiles = [] }) => this.uploadNoteMedia(tempFiles.map((file) => file.tempFilePath).filter(Boolean), "image") });
  },
  chooseNoteVideo() {
    wx.chooseMedia({
      count: 1,
  ...
  async uploadNoteMedia(paths, type) {
    ...
    try {
      ...
      const nextItems = uploaded
        ...
        .map((item, index) => ({
          id: item.id,
          type: item.type || item.mediaType || type,
          url: item.url,
          name: item.name,
  ...
  applyMediaState(form) {
    ...
    form.visibilityConfig = config;
    const mediaItems = buildMediaItems(form);
    const isImageNote = isImagePrimaryNote({ ...form, visibilityConfig: config });
    const imageCaption = isImageNote
      ? cleanImagePrimaryText(form.body || structuredData.rawText || (structuredData.ocr || {}).text || "")
      : "";
    if (isImageNote) {
      form.title = imagePrimaryTitle(form, imageCaption);
      form.body = imageCaption;
      form.summary = cleanImagePrimaryText(form.summary) || imageCaption.slice(0, 120);
    }
    const imageContentItems = mediaItems.filter((item) => item.type === "image");
    const attachmentMediaItems = isImageNote ? mediaItems.filter((item) => item.type !== "image") : mediaItems;
    this.setData({
      form,
      structuredData,
      mediaItems,
      ...
      imageContentItems,
      attachmentMediaItems,
      isPlainNote: isImageNote ? false : this.data.isPlainNote,
      displayTitle: isImageNote ? form.title : this.data.displayTitle,
      displaySubtitle: isImageNote ? (imageCaption !== form.title ? imageCaption : "") : this.data.displaySubtitle,
      businessCardImages: buildBusinessCardImageState(form, structuredData),
      ...
      mediaCountText: mediaItems.length ? `共 ${mediaItems.length} 个素材，可隐藏但不会删除。` : "暂无素材，可稍后补充。",
      plainAttachmentsOpen: this.data.isPlainNote ? mediaItems.length > 0 : this.data.plainAttachmentsOpen,
      "form.visibilityConfig": config
    });
  },
  ...
  async handleSaveOnly() {
    ...
    delete visibilityConfig.marketingRoute;
    const nextForm = {
      ...form,
      visibilityConfig
    };
    ...
    await api.updateNote(noteId, {
      ...
      ...nextForm,
      title,
      body: cardType === "text_note" ? nextForm.body.trim() : nextForm.body.trim() || title,
      contentBlocks: buildContentBlocks({
        ...nextForm,
        body: cardType === "text_note" ? nextForm.body.trim() : nextForm.body.trim() || title
      })
    });
    resourceStore.invalidateOwner(user.id);
  },
  initFloatingSave() {
    try {
  ...
  onShareAppMessage() {
    ...
    const shareId = `share_${this.data.noteId || "note"}_${Date.now()}_${Math.floor(Math.random() * 100000)}`;
    const shareNote = {
      ...(this.data.form || {}),
      id: this.data.noteId || (this.data.form || {}).id,
      revision: this.data.revision
    };
    const shareState = getNoteShareSnapshotState(shareNote, user ? user.id : "", user || {});
    const persistedImage = getShareImageUrlFromState(shareState);
    if (!persistedImage) {
    ...
    return {
      title,
      path: `/pages/note-preview/index?id=${encodeURIComponent(this.data.noteId || "")}&sid=${encodeURIComponent(shareId)}&from=${encodeURIComponent(shareFromUserId)}&src=note_edit_share`,
