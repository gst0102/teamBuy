...
const { SHOWCASE_TEMPLATES, getShowcaseTemplate, getTemplatesForScene, getDefaultTemplateId, normalizeSceneType, normalizeTemplateId } = require("../../../utils/showcase-templates");
const {
...
const SHOWCASE_SHARE_CANVAS_ID = "showcaseEditShareCanvas";
...
function templateOptionsForScene(sceneType) {
  return getTemplatesForScene(sceneType).map((item) => ({
    ...item,
    previewFailed: false
  }));
}
...
Page({
  data: {
    ...
    templateOptions: templateOptionsForScene("notes"),
    ...
    // 创建合集默认不隐式加入资料；只有用户明确选择“从当前筛选生成”时才批量加入。
    activeGenerationMethod: "manual",
    ...
    activeTemplateId: getDefaultTemplateId("notes"),
    activeTemplate: getShowcaseTemplate(getDefaultTemplateId("notes")),
    activeCategory: "房产",
    categoryOptions: [],
    noteViewMode: "list",
  ...
  onLoad(options) {
    ...
    const modeScene = normalizeSceneType(options.sceneType || options.mode || (modeCategory === "房产" ? "property" : modeCategory === "商品" ? "groupbuy" : modeCategory === "服务" ? "service" : "notes"));
    const defaultTemplateId = getDefaultTemplateId(modeScene);
    const conditionFilter = readConditionFilter();
    this.setData({
      id: options.id || "",
      sceneType: modeScene,
      templateOptions: templateOptionsForScene(modeScene),
      activeTemplateId: defaultTemplateId,
      activeTemplate: getShowcaseTemplate(defaultTemplateId),
      ...
      activeGenerationMethod: options.method === "condition" || conditionFilter ? "condition" : options.method === "filter" ? "filter" : options.method === "radar_compare" ? "manual" : "manual",
  ...
  async loadAll() {
    ...
    try {
      let selectedItems = [];
      let activeTemplateId = getDefaultTemplateId(this.data.sceneType);
      ...
      if (id) {
        ...
        const sceneType = normalizeSceneType(page.sceneType || display.sceneType || this.data.sceneType);
        activeTemplateId = normalizeTemplateId(sceneType, page.templateId || getDefaultTemplateId(sceneType));
        ...
        this.setData({
          ...
          snapshotVersion: page.snapshotVersion || 0,
          shareSnapshot: page.shareSnapshot || {},
          sceneType,
          templateOptions: templateOptionsForScene(sceneType),
          unpublishedChanges: false,
          name: page.name || "",
          ...
          activeGenerationMethod: display.generationMethod || "manual",
      ...
      if (!loadedFromServer) {
        if (this.data.preselectNoteId) {
          ...
          selectedItems = this.data.activeGenerationMethod === "condition"
            ...
            : this.data.activeGenerationMethod === "filter"
      ...
      if (this.data.status === "published" && !this.data.unpublishedChanges) {
        this.scheduleShareImageGeneration();
      }
    } catch (error) {
  ...
  scheduleShareImageGeneration() {
    const run = () => this.prepareShareImage();
  ...
  async prepareShareImage() {
    ...
    try {
      ...
      const entity = {
        ...
        snapshotVersion: this.data.snapshotVersion || 0,
        shareSnapshot: this.data.shareSnapshot || {}
      };
      const fingerprint = createShareSnapshotFingerprint("showcase", id, getShareSourceRevision("showcase", entity), SHARE_CARD_STYLE_VERSION, source);
      ...
      if (token !== this.shareImageGenerationToken || this.data.id !== id || this.data.unpublishedChanges) return;
      if (imagePath) {
        this.setData({
      ...
      if (token === this.shareImageGenerationToken) this.setData({ shareImageGenerating: false });
    }
  },
  handlePrepareShare() {
    if (this.data.shareImageGenerating) return;
  ...
  handleTemplateSelect(event) {
    const templateId = normalizeTemplateId(this.data.sceneType, event.currentTarget.dataset.id);
  ...
  handleTemplatePreviewError(event) {
    const templateId = event.currentTarget.dataset.id;
    if (!templateId) return;
    this.setData({
      templateOptions: this.data.templateOptions.map((item) => (
        item.id === templateId ? { ...item, previewFailed: true } : item
      ))
    });
  },
  ...
  buildPayload() {
    const { user, name, description, bannerUrl, shareTitle, phone, wechat, contactText, selectedItems, activeTemplateId, activeCategory, activeGenerationMethod, displayLayout, notes, sceneType } = this.data;
    const template = getShowcaseTemplate(activeTemplateId);
  ...
  async saveDraft() {
    ...
    try {
      ...
      this.setData({
        id: page.id,
        status: page.status || "draft",
        updatedAt: page.updatedAt || this.data.updatedAt || "",
        snapshotVersion: page.snapshotVersion || this.data.snapshotVersion || 0,
        shareSnapshot: page.shareSnapshot || this.data.shareSnapshot || {}
      });
  ...
  async handlePublish() {
    try {
      ...
      this.setData({
        ...
        updatedAt: res.data.updatedAt || this.data.updatedAt || "",
        snapshotVersion: res.data.snapshotVersion || this.data.snapshotVersion || 0,
        shareSnapshot: res.data.shareSnapshot || {},
        unpublishedChanges: false,
        shareImageUrl: "",
        shareImageReady: false,
        shareDirect: false,
  ...
  handleShareTap() {
    subscription.requestViewNotificationSubscription("showcase_edit_share");
  },
  onShareAppMessage() {
